...
func getConn(schema, etcdaddr, serviceName string, operationID string) *grpc.ClientConn {
	...
	if ok {
		stringdata1, _ := json.Marshal(*r.grpcClientConn)
		stringdata2, _ := json.Marshal(*r)
		log.Debug(operationID, "etcd key ",
			schema+serviceName, "value ",
	...
	stringdata1, _ := json.Marshal(*r.grpcClientConn)
	stringdata2, _ := json.Marshal(*r)
	log.Debug(operationID, "etcd key ", schema+serviceName, "value ", string(stringdata1), string(stringdata2))
...
func GetConfigConn(serviceName string, operationID string) *grpc.ClientConn {
	...
	//15
	if config.Config.RpcRegisterName.OpenImWeb3Js == serviceName {
		configPortList = config.Config.RpcPort.OpenImWeb3JsPort
	}
