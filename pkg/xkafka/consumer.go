...
func (c *Consumer) Start() {
	...
	go func() {
		...
		for {
			...
			if err := c.client.Consume(c.ctx, topics, h); err != nil {
				xlog.CErrorf("kafka consume failed: %s", err.Error())
	...
	<-h.ready
	xlog.CInfof("consumer up and running")
}
...
func NewConsumer(opt *Option, metrics metrics.Provider, tracer tracer.Provider) (*Consumer, error) {
	...
	if listener := ch.consumer.listeners[topic]; listener != nil {
		for message := range claim.Messages() {
			ch.trace(message, func() {
				xlog.CDebugf("Message claimed: value = %s, timestamp = %v, topic = %s",
...
func (ch *consumerHandler) listen(listener Listener, session sarama.ConsumerGroupSession, message *sarama.ConsumerMessage) {
	...
	defer func() {
		...
		if err != nil {
			...
			xlog.CErrorf("consumer handle error: %v, message: %s", err, json)
		}
...
func (ch *consumerHandler) batchListen(listener BatchListener, session sarama.ConsumerGroupSession, msgArr []*sarama.ConsumerMessage) {
	...
	defer func() {
		...
		if err != nil {
			xlog.CErrorf("batch consumer handle error: %v", err)
		}
