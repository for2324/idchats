...
// Send 是异步发送接口
func (p *Producer) Send(topic string, value []byte, cb Callback) {
...
// SyncSend 是同步发送接口。
func (p *Producer) SyncSend(ctx context.Context, topic string, value []byte) error {
...
func (p *Producer) handle(msg *sarama.ProducerMessage, err error) {
	xlog.CDebugf("receive send response %+v, error %v", msg, err)
...
func NewProducer(opt *Option, metrics metrics.Provider, tracer tracer.Provider) (*Producer, error) {
