...
const (
	...
	ChangeEmailVerifyCodePrefix   = "ChangeEmailVerifyCodePrefix"
)
...
func (d *DataBases) SetEmailVerifyCodeV2(key string, code, ttl int) (err error) {
	return d.RDB.Set(context.Background(), key, code, time.Duration(ttl)*time.Second).Err()
}
func (d *DataBases) GetEmailVerifyCodeV2(key string) (string, error) {
	return d.RDB.Get(context.Background(), key).Result()
}
func (d *DataBases) DeleteEmailVerifyCodeV2(key string) (int64, error) {
	return d.RDB.Del(context.Background(), key).Result()
}
...
func (d *DataBases) DelAccountCode(account string) (uint64, error) {
	key := accountTempCode + account
	return d.RDB.Del(context.Background(), key).Uint64()
}

func (d *DataBases) SetAccountNonce(account string, code string, ttl int) (err error) {
	key := accountTempNonce + account
	return d.RDB.Set(context.Background(), key, code, time.Duration(ttl)*time.Second).Err()
}
func (d *DataBases) SetAccountThirdPlatformString(account string, partstring string, content string, ttl int) (err error) {
	key := ThirdStringUser + account + ":" + partstring
	return d.RDB.Set(context.Background(), key, content, time.Duration(ttl)*time.Second).Err()
}
func (d *DataBases) GetAccountThirdPlatformString(account string, partstring string) (string, error) {
	key := ThirdStringUser + account + ":" + partstring
	return d.RDB.Get(context.Background(), key).Result()
}

func (d *DataBases) GetAccountNonce(account string) (string, error) {
	key := accountTempNonce + account
	return d.RDB.Get(context.Background(), key).Result()
}
...
func (d *DataBases) SetUserGlobalAnnouncementRecvOpt(userID string, opt int32) error {
	key := conversationReceiveMessageOpt + userID
	return d.RDB.HSet(context.Background(), key, GlobalAnnouncement, opt).Err()
}
func (d *DataBases) GetUserGlobalAnnouncementRecvOpt(userID string) (int, error) {
	key := conversationReceiveMessageOpt + userID
	result, err := d.RDB.HGet(context.Background(), key, GlobalAnnouncement).Result()
	if err != nil {
		if err == go_redis.Nil {
			return 0, nil
		} else {
...
func (d *DataBases) GetSyncEventMax() (uint64, error) {
	result, err := d.RDB.Get(context.Background(), "SYNCEVENT_TOKEN").Uint64()
	return result, err
}
func (d *DataBases) SetSyncEventMax(value uint64) error {
	err := d.RDB.Set(context.Background(), "SYNCEVENT_TOKEN", value, 0).Err()
	return err
}
...
func (d *DataBases) SetFileUpdateTime(filename string) error {
	return d.RDB.Set(context.Background(), "FILE_"+filename, time.Now().Format("2006-01-02 15:04:05"), 0).Err()
}
func (d *DataBases) GetFileUpdateTime(filename string) (string, error) {
	return d.RDB.Get(context.Background(), "FILE_"+filename).Result()
}

func (d *DataBases) SetUserCheckIn(userId string) error {
	key := "USER_CHECK_IN_" + time.Now().Format("20060102")
	if err := d.RDB.ExpireNX(context.Background(), key, time.Hour*24).Err(); err != nil {
		log2.NewError("SetUserCheckIn", "ExpireNX failed", err.Error())
	}
	return d.RDB.SAdd(context.Background(), key, userId).Err()
}
func (d *DataBases) GetUserIsCheckIn(userId string) (bool, error) {
	key := "USER_CHECK_IN_" + time.Now().Format("20060102")
	return d.RDB.SIsMember(context.Background(), key, userId).Result()
}
