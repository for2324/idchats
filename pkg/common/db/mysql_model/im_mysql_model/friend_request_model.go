...
func FollowInsertFriendApplication(friendRequest *db.UserFollow, args map[string]interface{}) error {

	if friendRequest.Follow == 0 {
		db.DB.MysqlDB.DefaultGormDB().Table("user_follow").Unscoped().
			Delete(&db.UserFollow{
				FromUserID:   friendRequest.FromUserID,
				FollowUserID: friendRequest.FollowUserID,
			})
		return nil
	} else {
		var tempdata db.UserFollow
		db.DB.MysqlDB.DefaultGormDB().Table("user_follow").Where("from_user_id=? and follow_user_id=? ",
			friendRequest.FromUserID, friendRequest.FollowUserID).First(&tempdata)
		if tempdata.FromUserID != friendRequest.FromUserID {
			return db.DB.MysqlDB.DefaultGormDB().Table("user_follow").Create(friendRequest).Error
		}
	}
	return nil
}
