...
func GetGroupMemberListByGroupID(groupID string) ([]db.GroupMember, error) {
	var groupMemberList []db.GroupMember
	err := db.DB.MysqlDB.DefaultGormDB().Table("group_members").Where("group_id=?", groupID).
...
func GetGroupMemberListByGroupIDMerlin(groupID string) ([]db.GroupMember, error) {
	var groupMemberList []db.GroupMember
	chainId := 1
	if !config.Config.IsPublicEnv {
		chainId = 5
	}
	err := db.DB.MysqlDB.DefaultGormDB().Raw(`select group_id,group_members.user_id,role_level,join_time,join_source,inviter_user_id,operator_user_id,mute_end_time,group_members.ex,
users.face_url as user_group_face_url,ens_domain as nickname from group_members 
left join   users  on group_members.user_id = users.user_id
left join user_domains on group_members.user_id = user_domains.user_id and user_domains.chain_id = ?
where group_id=? `, chainId, groupID).Find(&groupMemberList).Error
	if err != nil {
		return nil, err
	}
	return groupMemberList, nil
}
func GetGroupMemberListByGroupIDAndRoleLevelMerlin(groupID string, roleLevel int32) ([]db.GroupMember, error) {
...
left join   users  on group_members.user_id = users.user_id
left join user_domains on group_members.user_id = user_domains.user_id and user_domains.chain_id = ?
where group_id=? and role_level=?`, chainId, groupID, roleLevel).Find(&groupMemberList).Error
	if err != nil {
		return nil, err
	}
	return groupMemberList, nil
...
func GetGroupMemberInfoByGroupIDAndUserIDMerlin(groupID, userID string) (*db.GroupMember, error) {
...
left join   users  on group_members.user_id = users.user_id
left join user_domains on group_members.user_id = user_domains.user_id and user_domains.chain_id = ?
where group_id=? and group_members.user_id=?`, chainId, groupID, userID).Limit(1).Take(&groupMember).Error

	//
	//err := db.DB.MysqlDB.DefaultGormDB().Table("group_members").
	//	Where("group_id=? and user_id=? ", groupID, userID).Limit(1).Take(&groupMember).Error
	if err != nil {
		return nil, err
	}
...
func GetGroupMemberInfoByGroupIDAndUserID(groupID, userID string) (*db.GroupMember, error) {
	...
	//left join   users  on group_members.user_id = users.user_id
	//left join user_domains on group_members.user_id = user_domains.user_id and user_domains.chain_id = ?
	//where group_id=? and user_id=?`, chainId, groupID, userID).Limit(1).Take(&groupMember).Error
...
func DeleteGroupMemberByGroupIDAndUserID(groupID, userID string) error {
	return db.DB.MysqlDB.DefaultGormDB().Table("group_members").
		Where("group_id=? and user_id=? ", groupID, userID).Delete(db.GroupMember{}).Error
}
...
func GetGroupCountByUserId(userID string) (count int64, err error) {
	//select count(*) from groups where creator_user_id='0xe5a9748cab1a4a5756563c0eb4a281a9345e0acd' and `status`=0 and group_type = 2
	err = db.DB.MysqlDB.DefaultGormDB().Table("groups").
		Where("creator_user_id =? and status=0 and group_type=?", userID, constant.WorkingGroup).Count(&count).Error
	return
}
...
func GetGroupMemberByGroupID(groupID string, filter int32, begin int32, maxNumber int32) ([]db.GroupMember, error) {
	...
	var err error
	if filter >= 0 {
		memberList, err = GetGroupMemberListByGroupIDAndRoleLevelMerlin(groupID, filter) //sorted by join time
	} else {
		memberList, err = GetGroupMemberListByGroupIDMerlin(groupID)
	}
