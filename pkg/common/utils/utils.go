...
func FriendDBCopyOpenIMByChainID(dst *open_im_sdk.FriendInfo, src *db.Friend, chanid string) error {
	utils.CopyStructFields(dst, src)
	user, err := imdb.GetUserByUserID(src.FriendUserID)
	if err != nil {
		return utils.Wrap(err, "")
	...
	utils.CopyStructFields(dst.FriendUser, user)
	var selectChainEnsName = chanid
	if chanid == "" {
		selectChainEnsName = utils.Int32ToString(1)
	}
	resultName, _ := imdb.GetUserEnsNameByChainID(src.FriendUserID, selectChainEnsName)
	if resultName != "" {
		dst.FriendUser.Nickname = resultName
	}
	dst.CreateTime = uint32(src.CreateTime.Unix())
	if dst.FriendUser == nil {
		dst.FriendUser = &open_im_sdk.UserInfo{}
...
func GroupChannelDBCopyOpenIM(dst *open_im_sdk.GroupChannelInfo, src *db.GroupChannel) error {
	utils.CopyStructFields(dst, src)
	return nil
}
