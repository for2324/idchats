...
func init() {
	logger = loggerInit(constant.LogFileName)
...
func loggerInit(moduleName string) *Logger {
	...
	hook := NewLfsHook(time.Duration(config.Config.Log.RotationTime)*
		time.Hour, config.Config.Log.RemainRotationCount, moduleName)
	logger.AddHook(hook)
	return &Logger{
		logger,
		os.Getpid(),
