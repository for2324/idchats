...
import (
	"Open_IM/pkg/biubiuens/contract/reverseresolver"
	"errors"
	"fmt"
...
func NewReverseResolver(backend bind.ContractBackend) (*ReverseResolver, error) {
	reverseRegistrardata, err := NewReverseRegistrar(backend)
	if err != nil {
		return nil, err
	}

	// Now fetch the default resolver
	address, err := reverseRegistrardata.DefaultResolverAddress()
	if err != nil {
		return nil, err
	}
