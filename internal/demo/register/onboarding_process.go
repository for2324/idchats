...
var ChOnNewLogin chan OnboardingProcessReq

func init() {
	Ch = make(chan OnboardingProcessReq, 1000)
	ChOnNewLogin = make(chan OnboardingProcessReq, 1000)
}
...
func OnboardingLoginRoutine() {
	for {
		req := <-ChOnNewLogin
		go func() {
			onboardingLoginProcess(req.OperationID, req.UserID, req.NickName, req.FaceURL, req.PhoneNumber, req.Email)
		}()
	}
}

func onboardingLoginProcess(operationID, userID, userName, faceURL, phoneNumber, email string) {
...
func onboardingProcessNotification(operationID, userID, groupID, userName, faceURL string) {
	...
	//	return
	//}
	welcomeString := fmt.Sprintf("welcome:%s", userName)
