...
func MsgToSuperGroupUser(pushMsg *pbPush.PushMsgReq) {
	...
	grpcCons := getcdv3.GetDefaultGatewayConn4Unique(config.Config.Etcd.EtcdSchema,
		strings.Join(config.Config.Etcd.EtcdAddr, ","), pushMsg.OperationID)
	...
	log.NewInfo(pushMsg.OperationID, "push_result", wsResult, "sendData", utils.StructToJsonString(pushMsg.MsgData))
	successCount++
	if isOfflinePush {
