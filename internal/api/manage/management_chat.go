...
type AnnouncementElem struct {
	AnnouncementUrl string `mapstructure:"announcementUrl"` //在ipfs的公告的信息 url
	//文字、文字+视频、文字+图片、文字+图片+链接、文字+链接、图片、链接（单独一条外部链接）
	Text          string   `mapstructure:"text"` //文字内容
	VideoPath     []string `mapstructure:"videoPath"`
	PicturePath   []string `mapstructure:"videoPath"`
	UrlPath       []string `mapstructure:"urlPath"` //是否可以对url鉴权
	IsGlobalWorld bool     `mapstructure:"isGlobalWorld"`
}
