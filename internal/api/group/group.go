...
func GetGroupMemberList(c *gin.Context) {
	...
	memberListResp := api.GetGroupMemberListResp{CommResp: api.CommResp{ErrCode: RpcResp.ErrCode,
		ErrMsg: RpcResp.ErrMsg}, MemberList: RpcResp.MemberList, NextSeq: RpcResp.NextSeq}
...
func GetGroupAllMemberList(c *gin.Context) {
	...
	memberListResp := api.GetGroupAllMemberResp{CommResp: api.CommResp{ErrCode: RpcResp.ErrCode, ErrMsg: RpcResp.ErrMsg},
		MemberList: RpcResp.MemberList}
	memberListResp.Data = jsonData.JsonDataList(memberListResp.MemberList)
...
func CreateGroup(c *gin.Context) {
	...
	if strings.Trim(params.GroupName, " ") == "" {
		c.JSON(http.StatusOK, gin.H{"errCode": 400, "errMsg": "group name can't space"})
		return
	}
...
func CreateSysUserGroup(c *gin.Context) {
	var (
		req  api.CreateSysUserGroupReq
		resp api.CreateSysUserResp
	)
	...
	client := rpc.NewGroupClient(etcdConn)
	userIds := config.Config.InitUser.UserId
	respPb, err := client.CreateSysUserGroup(context.Background(), &rpc.CreateSysUserGroupReq{
		UserId: userIds,
	})
	if err != nil {
		log.NewError(req.OperationID, utils.GetSelfFuncName(), " failed ", err.Error())
...
// IsCanGetMemberCountReword
// @Summary		获取是否可以领取本社区的奖励
// @Description	获取是否可以领取本社区的奖励
// @Tags			群组相关
// @ID				IsCanGetMemberCountReword
// @Accept			json
// @Param			token	header	string						true	"im token"
// @Param			req		body	api.GetCanRewordMemberCountRewordReq	true	"groupID为要获取的群ID <br> "
// @Produce		json
// @Success		0	{object}	api.GetCanRewordMemberCountRewordResp
// @Failure		500	{object}	api.Swagger500Resp	"errCode为500 一般为服务器内部错误"
// @Failure		400	{object}	api.Swagger400Resp	"errCode为400 一般为参数输入错误, token未带上等"
// @Router			/group/is_can_get_member_count_reword [post]
func IsCanGetMemberCountReword(c *gin.Context) {
	params := api.GetCanRewordMemberCountRewordReq{}
	if err := c.BindJSON(&params); err != nil {
		log.NewError("0", "BindJSON failed ", err.Error())
		c.JSON(http.StatusBadRequest, gin.H{"errCode": 400, "errMsg": err.Error()})
		return
	}
	req := &rpc.GetGroupHaveNftMemberCountReq{}
	req.OperationID = params.OperationID
	...
	client := rpc.NewGroupClient(etcdConn)
	RpcResp, err := client.IsCanGetGroupMemberHaveNftCountReword(context.Background(), req)
	if err != nil {
		log.NewError(req.OperationID, utils.GetSelfFuncName(), " failed ", err.Error())
		c.JSON(http.StatusInternalServerError, gin.H{"errCode": 500, "errMsg": err.Error()})
		return
...
// @Accept			json
// @Param			token	header	string						true	"im token"
// @Param			req		body	api.ColligateSearchReq	true	"keyword为要搜索的关键字 <br>"
// @Produce		json
// @Success		0	{object}	api.ColligateSearchResp
// @Failure		500	{object}	api.Swagger500Resp	"errCode为500 一般为服务器内部错误"
// @Failure		400	{object}	api.Swagger400Resp	"errCode为400 一般为参数输入错误, token未带上等"
// @Router			/group/colligate_search [post]
func ColligateSearch(c *gin.Context) {
	params := api.ColligateSearchReq{}
	if err := c.BindJSON(&params); err != nil {
		log.NewError("0", "BindJSON failed ", err.Error())
		c.JSON(http.StatusBadRequest, gin.H{"errCode": 400, "errMsg": err.Error()})
		return
	}
	req := &rpc.ColligateSearchReq{}
	utils.CopyStructFields(req, &params)
	var ok bool
	var errInfo string
	ok, req.OpUserID, errInfo = token_verify.GetUserIDFromToken(c.Request.Header.Get("token"), req.OperationID)
	...
	client := rpc.NewGroupClient(etcdConn)
	RpcResp, err := client.ColligateSearch(context.Background(), req)
	if err != nil {
		log.NewError(req.OperationID, utils.GetSelfFuncName(), " failed ", err.Error())
		c.JSON(http.StatusInternalServerError, gin.H{"errCode": 500, "errMsg": err.Error()})
		return
	}
	resp := api.ColligateSearchResp{}
	utils.CopyStructFields(&resp.Data, RpcResp.ColligateSearchBody)
	c.JSON(http.StatusOK, gin.H{"errCode": RpcResp.CommonResp.ErrCode, "errMsg": RpcResp.CommonResp.ErrMsg, "data": resp.Data})
}

// CreatePushSpaceArticelOrder
// @Summary		创建全网推送文章订单(转发别人的文章创建全局推送)
// @Description	创建全网推送文章订单(转发别人的文章创建全局推送)
// @Tags			群组相关
// @ID				CreatePushSpaceArticelOrder
// @Accept			json
// @Param			token	header	string						true	"im token"
// @Param			req		body	api.CreatePushSpaceArticelOrderReq	true	" <br>"
// @Produce		json
// @Success		0	{object}	api.CreatePushSpaceArticelOrderResp
// @Failure		500	{object}	api.Swagger500Resp	"errCode为500 一般为服务器内部错误"
// @Failure		400	{object}	api.Swagger400Resp	"errCode为400 一般为参数输入错误, token未带上等"
// @Router			/group/create_push_space_articel_order [post]
func CreatePushSpaceArticelOrder(c *gin.Context) {
	params := api.CreatePushSpaceArticelOrderReq{}
	if err := c.BindJSON(&params); err != nil {
		log.NewError("0", "BindJSON failed ", err.Error())
		c.JSON(http.StatusBadRequest, gin.H{"errCode": 400, "errMsg": err.Error()})
		return
	}
	req := &rpc.CreatePushSpaceArticelOrderReq{}
	utils.CopyStructFields(req, &params)
	var ok bool
	var errInfo string
	...
	client := rpc.NewGroupClient(etcdConn)
	RpcResp, err := client.CreatePushSpaceArticelOrder(context.Background(), req)
	if err != nil {
		log.NewError(req.OperationID, utils.GetSelfFuncName(), " failed ", err.Error())
		c.JSON(http.StatusInternalServerError, gin.H{"errCode": 500, "errMsg": err.Error()})
		return
	}
	resp := api.CreatePushSpaceArticelOrderResp{}
	utils.CopyStructFields(&resp.Data, RpcResp.PayInfo)
	c.JSON(http.StatusOK, gin.H{"errCode": RpcResp.CommonResp.ErrCode, "errMsg": RpcResp.CommonResp.ErrMsg, "data": resp.Data})
}
