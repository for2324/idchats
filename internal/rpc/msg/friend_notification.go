...
func getFromToUserNickname(fromUserID, toUserID string) (string, string, error) {
	...
	return from.Nickname, tonickname, nil
}
...
func friendNotification(commID *pbFriend.CommID, contentType int32, m proto.Message) {
	...
	case constant.FriendFollowApplicationNotification:
		tips.DefaultTips = fromUserNickname + cn.FriendFollowApplication.DefaultTips.Tips
	case constant.FriendFollowDeleteApplicationNotification:
		tips.DefaultTips = fromUserNickname + cn.FriendFollowDeleteApplication.DefaultTips.Tips
...
func FriendFollowApplicationNotification(req *pbFriend.FollowAddFriendReq) {
	fmt.Println("1>>>>>>>>>>>>>>>>", utils.StructToJsonString(req.CommID))
	friendAddedTips := open_im_sdk.FollowFriendTips{
		Follow: req.Follow,
	}
	user, err := imdb.GetUserByUserID(req.CommID.FromUserID)
	if err != nil {
		return
	}
	tempRequestPublicUserInfo := new(open_im_sdk.PublicUserInfo)
	utils2.UserDBCopyOpenIMPublicUser(tempRequestPublicUserInfo, user)
	friendAddedTips.PublicUserInfo = append(friendAddedTips.PublicUserInfo, tempRequestPublicUserInfo)
	commID := pbFriend.CommID{FromUserID: req.CommID.FromUserID,
		ToUserID: req.CommID.ToUserID, OpUserID: req.CommID.OpUserID,
		OperationID: req.CommID.OperationID}
	friendNotification(&commID, constant.FriendFollowApplicationNotification, &friendAddedTips)
}
func FriendFollowDeleteApplicationNotification(req *pbFriend.FollowAddFriendReq) {
	fmt.Println("1>>>>>>>>>>>>>>>>", utils.StructToJsonString(req.CommID))
	friendAddedTips := open_im_sdk.FollowFriendTips{
		Follow: req.Follow,
	}
	...
	commID := pbFriend.CommID{FromUserID: req.CommID.FromUserID,
		ToUserID: req.CommID.ToUserID, OpUserID: req.CommID.OpUserID,
		OperationID: req.CommID.OperationID}
	friendNotification(&commID, constant.FriendFollowDeleteApplicationNotification, &friendAddedTips)
}
...
func FriendAddedNotification(operationID, opUserID, fromUserID, toUserID string) {
	...
	useropinfo, err := rocksCache.GetUserBaseInfoFromCache(toUserID)
	utils2.FriendDBCopyOpenIMByChainID(friendAddedTips.Friend, friend, utils.Int32ToString(useropinfo.Chainid))
