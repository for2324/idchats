...
type rpcChat struct {
	...
	messageWriterLikeEvent MessageWriter
...
func NewRpcChatServer(port int) *rpcChat {
	...
	rc.messageWriterLikeEvent = kafka.NewKafkaProducer(config.Config.Kafka.LikesAction.Addr, config.Config.Kafka.LikesAction.Topic)
