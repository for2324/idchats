...
func setGroupChannel(groupid string, channelid string, channelinfo *open_im_sdk.GroupChannelInfo) error {
	groupChannel, err := imdb.GetGroupChannelInfo(groupid, channelid)
	if err != nil {
		return utils.Wrap(err, "setGroupChannel failed")
	}
	err = utils2.GroupChannelDBCopyOpenIM(channelinfo, groupChannel)
	if err != nil {
		log.NewWarn("", "setGroupChannel failed ", groupid, channelid, err.Error())
		return nil
	}
	return nil
...
func groupNotification(contentType int32, m proto.Message, sendID, groupID, recvUserID, operationID string, channelId string) {

	log.Info(operationID, utils.GetSelfFuncName(), "args: ", contentType, sendID, groupID, recvUserID, channelId)
	if channelId == "" {
		channelId = "2"
	}
	var err error
	...
	//	if err != nil {
	//		log.NewWarn(operationID,
	//			"GetUserByUserID failed ",
	//			err.Error(), recvUserID)
	//	}
	//	if to != nil {
	...
	//		"recvUserID ==nil ,and  this message is :", contentType)
	//}
	...
	//	tips.DefaultTips = toNickname + "" + "new channel create"
	//case constant.GroupChannelInfoDiss:
	//	tips.DefaultTips = toNickname + "" + "channel delete"
	//case constant.GroupChannelInfoUpdate:
	//	tips.DefaultTips = toNickname + "" + "channel update"
	//default:
...
// GroupInfoSetNotification 群信息改变后掉用
// groupName := ""
//
//	notification := ""
//	introduction := ""
...
func GroupInfoSetNotification(operationID, opUserID, groupID string, groupName, notification, introduction, faceURL string,
	needVerification *wrapperspb.Int32Value) {
	GroupInfoChangedTips := open_im_sdk.GroupInfoSetTips{Group: &open_im_sdk.GroupInfo{},
...
func GroupChannelNotification(operationID, opUserID, groupID string, channelname string, channelid string, opInfo string) {
	GroupInfoChangedTips := open_im_sdk.GroupChannelInfoTips{
		OpUser: &open_im_sdk.GroupMemberFullInfo{}, OpInfo: opInfo,
		GroupChannelInfoTip: &open_im_sdk.GroupChannelInfo{}}
	if err := setOpUserInfo(opUserID, groupID, GroupInfoChangedTips.OpUser); err != nil {
		log.Error(operationID, "setOpUserInfo failed ", err.Error(), opUserID, groupID)
		return
	}
	if err := setGroupChannel(groupID, channelid, GroupInfoChangedTips.GroupChannelInfoTip); err != nil {
		log.Error(operationID, "setOpUserInfo failed ", err.Error(), opUserID, groupID)
		return
	}
	switch opInfo {
	case "add":
		groupNotification(constant.GroupChannelInfoAdd, &GroupInfoChangedTips, opUserID, groupID, "", operationID, channelid)
	case "del":
		groupNotification(constant.GroupChannelInfoDiss, &GroupInfoChangedTips, opUserID, groupID, "", operationID, channelid)
	case "update":
		groupNotification(constant.GroupChannelInfoUpdate, &GroupInfoChangedTips, opUserID, groupID, "", operationID, channelid)
