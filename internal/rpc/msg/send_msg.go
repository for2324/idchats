...
func (rpc *rpcChat) SendMsg(_ context.Context, pb *pbChat.SendMsgReq) (*pbChat.SendMsgResp, error) {
	...
	fmt.Println("update chat data:", utils.StructToJsonString(pb.MsgData))
	...
	case constant.SingleChatType:
		...
		fmt.Println(pb.OperationID, "modifyMessageByUserMessageReceiveOpt ", " cost time: ", time.Since(t1))
		if isSend {
		...
		if !sendTag {
			...
			if pb.MsgData.ContentType == constant.AtText {
				go func() {
					...
					var tag bool
					var atUserID []string
					conversionid := utils.GetConversationIDBySessionType(pb.MsgData.GroupID, constant.GroupChatType)
					conversionType := constant.GroupChatType
					if pb.MsgData.ChannelID != "" {
						conversionid = utils.GetConversationIDBySessionType(pb.MsgData.GroupID, constant.SuperGroupChatType)
						conversionType = constant.SuperGroupChatType
					}
					conversation := pbConversation.Conversation{
						OwnerUserID:      pb.MsgData.SendID,
						ConversationID:   conversionid,
						ConversationType: int32(conversionType),
	...
	case constant.SuperGroupChatType:
		...
		msgToMQSingle.MsgData = pb.MsgData
		log.NewInfo(msgToMQSingle.OperationID, msgToMQSingle)
		err1 := rpc.sendMsgToWriter(&msgToMQSingle, msgToMQSingle.MsgData.GroupID,
			constant.OnlineStatus)
		if err1 != nil {
			log.NewError(msgToMQSingle.OperationID, "kafka send msg err:RecvID", msgToMQSingle.MsgData.RecvID, msgToMQSingle.String())
			promePkg.PromeInc(promePkg.WorkSuperGroupChatMsgProcessFailedCounter)
			return returnMsg(&replay, pb, 201, "kafka send msg err", "", 0)
...
func (rpc *rpcChat) sendMsgToWriter(m *pbChat.MsgDataToMQ, key string, status string) error {
	...
	case constant.OnlineStatus:
		...
		log.Info(m.OperationID, "kafka send message to sendMsgToWriter", key, m.String())
...
func Notification(n *NotificationMsg) {
	...
	case constant.ConversationUnreadNotification, constant.SuperGroupUpdateNotification, constant.ConversationMonitorForGroupNotification:
...
func (rpc *rpcChat) sendMsgToGroup(list []string, pb pbChat.SendMsgReq, status string, sendTag *bool, wg *sync.WaitGroup) {
	...
	for _, v := range list {
		...
		isSend := modifyMessageByUserMessageReceiveOpt(v, msgData.GroupID, sessionType, &groupPB)
		if isSend {
...
func (rpc *rpcChat) sendMsgToGroupOptimization(list []string, groupPB *pbChat.SendMsgReq, status string, sendTag *bool, wg *sync.WaitGroup) {
	...
	for _, v := range list {
		...
		groupPB.MsgData.Options = options
		sessionType := constant.GroupChatType
		if groupPB.MsgData.ChannelID != "" {
			sessionType = constant.SuperGroupChatType
		}
		isSend := modifyMessageByUserMessageReceiveOpt(v, groupPB.MsgData.GroupID, sessionType, groupPB)
		if isSend {
...
func (rpc *rpcChat) SendLikeAction(ctx context.Context, req *pbChat.SendLikeMsgReq) (*pbChat.SendLikeMsgResp, error) {
	MsgLikeMQ := &pbChat.MsgLikeMQ{
		Token:       req.Token,
		OperationID: req.OperationID,
		LikeReword:  req.MsgData,
	}
	//topic is config.Config.Kafka.LikesAction.Topic
	pid, offset, err := rpc.messageWriterLikeEvent.SendMessage(MsgLikeMQ, req.MsgData.ArticleID, req.OperationID)
	if err != nil {
		log.Error(req.OperationID, "kafka send failed", "send data", req.String(), "pid", pid, "offset", offset, "err", err.Error(), "key", req.MsgData.ArticleID)
	}
	return &pbChat.SendLikeMsgResp{
		ErrCode: 0,
		ErrMsg:  "",
	}, nil
}
