...
func callbackBeforeCreateGroup(req *pbGroup.CreateGroupReq) cbApi.CommonCallbackResp {
	...
	if err := http.CallBackPostReturn(config.Config.Callback.CallbackUrl,
		constant.CallbackBeforeCreateGroupCommand,
		commonCallbackReq, resp, config.Config.Callback.CallbackBeforeCreateGroup.CallbackTimeOut); err != nil {
