...
type groupServer struct {
	...
	etcdSchema       string
	etcdAddr         []string
	messagePushWrite *kafka.Producer
}
...
func NewGroupServer(port int) *groupServer {
	...
	return &groupServer{
		...
		messagePushWrite: kafka.NewKafkaProducer(config.Config.Kafka.AnnouncementAction.Addr, config.Config.Kafka.AnnouncementAction.Topic),
	}
}
...
func (s *groupServer) CreateGroup(ctx context.Context, req *pbGroup.CreateGroupReq) (*pbGroup.CreateGroupResp, error) {
	...
	if req.OwnerUserID != "" {
		...
		//to group member
		groupMember = db.GroupMember{GroupID: groupId,
			RoleLevel: constant.GroupOwner, OperatorUserID: req.OpUserID, JoinSource: constant.JoinByInvitation, InviterUserID: req.OpUserID}
	...
	if req.GroupInfo.GroupType != constant.SuperGroup {
		for _, user := range req.InitMemberList {
			us, err := rocksCache.GetUserBaseInfoFromCache(user.UserID)
			if err != nil {
...
func (s *groupServer) KickGroupMember(ctx context.Context, req *pbGroup.KickGroupMemberReq) (*pbGroup.KickGroupMemberResp, error) {
	...
	if groupInfo.GroupType != constant.SuperGroup {
		...
		conversationID := utils.GetConversationIDBySessionType(req.GroupID, constant.GroupChatType)
		if groupInfo.GroupType == constant.WorkingGroup {
			conversationID = utils.GetConversationIDBySessionType(req.GroupID, constant.SuperGroupChatType)
		}
		var reqPb pbUser.SetConversationReq
		var c pbConversation.Conversation
		for _, v := range okUserIDList {
...
func (s *groupServer) GroupApplicationResponse(_ context.Context, req *pbGroup.GroupApplicationResponseReq) (*pbGroup.GroupApplicationResponseResp, error) {
	...
	if req.HandleResult == constant.GroupResponseAgree {
		...
		reqPb.OperationID = req.OperationID
		var c pbConversation.Conversation
		conversionID := utils.GetConversationIDBySessionType(req.GroupID, constant.GroupChatType)
		dbGroup, err := imdb.GetGroupInfoByGroupID(req.GroupID)
		if dbGroup != nil && dbGroup.GroupType == constant.WorkingGroup {
			conversionID = utils.GetConversationIDBySessionType(req.GroupID, constant.SuperGroupChatType)

		}
		conversation, err := imdb.GetConversation(req.FromUserID, conversionID)

		if err != nil {
			c.OwnerUserID = req.FromUserID
			c.ConversationID = conversionID
			c.ConversationType = constant.GroupChatType
			c.GroupID = req.GroupID
			c.IsNotInGroup = false
		} else {
...
func (s *groupServer) JoinGroup(ctx context.Context, req *pbGroup.JoinGroupReq) (*pbGroup.JoinGroupResp, error) {
	...
	if groupInfo.NeedVerification == constant.Directly {
		if groupInfo.GroupType != constant.SuperGroup {
			...
			if groupInfo.CreatorUserID == req.OpUserID {
				return &pbGroup.JoinGroupResp{CommonResp: &pbGroup.CommonResp{ErrCode: constant.ErrDB.ErrCode, ErrMsg: "your are this group manager"}}, nil
			}
			etcdConn := getcdv3.GetDefaultConn(config.Config.Etcd.EtcdSchema, strings.Join(config.Config.Etcd.EtcdAddr, ","), config.Config.RpcRegisterName.OpenImCacheName, req.OperationID)
			if etcdConn == nil {
				errMsg := req.OperationID + "getcdv3.GetDefaultConn == nil"
			...
			UserIDList, err2 := rocksCache.GetGroupMemberIDListFromCache(req.GroupID)
			if err2 != nil {
				log.NewError(req.OperationID, "InsertIntoGroupMember failed ", err2.Error(), req.GroupID)
				return &pbGroup.JoinGroupResp{CommonResp: &pbGroup.CommonResp{ErrCode: constant.ErrDB.ErrCode, ErrMsg: constant.ErrDB.ErrMsg}}, nil
			}
			if utils.IsContain(req.OpUserID, UserIDList) {
				log.NewError(req.OperationID, "InsertIntoGroupMember failed ", req.GroupID)
				return &pbGroup.JoinGroupResp{CommonResp: &pbGroup.CommonResp{
					ErrCode: constant.ErrArgs.ErrCode, ErrMsg: "you had join this group"}}, nil
			}
			//to group member
			groupMember := db.GroupMember{GroupID: req.GroupID,
				RoleLevel: constant.GroupOrdinaryUsers, OperatorUserID: req.OpUserID}
			utils.CopyStructFields(&groupMember, us)
			err = imdb.InsertIntoGroupMember(groupMember)
			if err != nil {
				log.NewError(req.OperationID, "InsertIntoGroupMember failed ", err.Error(), groupMember)
			...
			// 自动关注群主
			if err := FollowAddFriend(req.OperationID, req.OpUserID, groupInfo.CreatorUserID); err != nil {
				log.NewError(req.OperationID, utils.GetSelfFuncName(), err.Error(), req.OpUserID, groupInfo.CreatorUserID)
			}

			err = rocksCache.DelGroupMemberIDListFromCache(req.GroupID)
			if err != nil {
				log.NewError(req.OperationID, "DelGroupMemberIDListFromCache rpc call failed ", err.Error())
				return &pbGroup.JoinGroupResp{CommonResp: &pbGroup.CommonResp{ErrCode: constant.ErrDB.ErrCode, ErrMsg: constant.ErrDB.ErrMsg}}, nil
			}
...
func (s *groupServer) QuitGroup(ctx context.Context, req *pbGroup.QuitGroupReq) (*pbGroup.QuitGroupResp, error) {
	log.NewInfo(req.OperationID, "QuitGroup args ", req.String(), time.Now().UnixNano())
	groupInfo, err := imdb.GetGroupInfoByGroupID(req.GroupID)
	if err != nil {
	...
	if groupInfo.GroupType != constant.SuperGroup {
		...
		if groupInfo.GroupType == constant.WorkingGroup {
			conversationID = utils.GetConversationIDBySessionType(req.GroupID, constant.SuperGroupChatType)
			c.ConversationType = constant.SuperGroupChatType
		}
		c.ConversationID = conversationID
		c.GroupID = req.GroupID
		c.IsNotInGroup = true
	...
	log.NewInfo(req.OperationID,
		"rpc QuitGroup return ", ">>ok", time.Now().UnixNano())
	return &pbGroup.QuitGroupResp{CommonResp: &pbGroup.CommonResp{ErrCode: 0, ErrMsg: ""}}, nil
}

func hasGroupChannelAccess(groupid string, user_id string, operatorID string) bool {
	groupUserInfo, err := imdb.GetGroupMemberInfoByGroupIDAndUserID(
		groupid, user_id)
	if err != nil {
		log.NewError(operatorID, "GetGroupMemberInfoByGroupIDAndUserID failed, ", err.Error(), groupid, user_id)
		return false
...
func (s *groupServer) SetGroupInfo(ctx context.Context, req *pbGroup.SetGroupInfoReq) (*pbGroup.SetGroupInfoResp, error) {
	...
	if changedType != 0 {
		chat.GroupInfoSetNotification(req.OperationID,
			req.OpUserID,
			...
			introduction,
			faceURL,
			req.GroupInfoForSet.NeedVerification)
	}
	if req.GroupInfoForSet.Notification != "" {
		//get group member user id
		getGroupMemberIDListFromCacheReq := &pbCache.GetGroupMemberIDListFromCacheReq{
			OperationID: req.OperationID,
		...
		if group.GroupType == constant.WorkingGroup {
			conversationID = utils.GetConversationIDBySessionType(req.GroupInfoForSet.GroupID, constant.SuperGroupChatType)
			conversationType = constant.SuperGroupChatType
		}
		conversation := pbConversation.Conversation{
			OwnerUserID:      req.OpUserID,
			ConversationID:   conversationID,
			ConversationType: int32(conversationType),
			GroupID:          req.GroupInfoForSet.GroupID,
		}
		...
		nEtcdConn := getcdv3.GetDefaultConn(config.Config.Etcd.EtcdSchema,
			strings.Join(config.Config.Etcd.EtcdAddr, ","),
			config.Config.RpcRegisterName.OpenImConversationName,
			req.OperationID)
		if etcdConn == nil {
			errMsg := req.OperationID + "getcdv3.GetDefaultConn == nil"
			log.NewError(req.OperationID, errMsg)
...
func (s *groupServer) DismissGroup(ctx context.Context, req *pbGroup.DismissGroupReq) (*pbGroup.DismissGroupResp, error) {
	...
	// 是否含有ido未结束的社区
	if resultflag, _ := checkisendidoproject(ctx, req.GroupID, req.OpUserID); !resultflag {
		return &pbGroup.DismissGroupResp{CommonResp: &pbGroup.CommonResp{ErrCode: constant.ErrInternal.ErrCode, ErrMsg: "ido not ending ,don't dismiss group"}}, nil
	}
	if dbinfo, err := imdb.GetGroupInfoByGroupID(req.GroupID); err != nil || dbinfo.Status != constant.GroupOk {
		return &pbGroup.DismissGroupResp{CommonResp: &pbGroup.CommonResp{ErrCode: constant.ErrInternal.ErrCode, ErrMsg: "don't dismiss the dismissed group"}}, nil
	}
	...
	if groupInfo.GroupType != constant.SuperGroup {
		memberList, err := imdb.GetGroupMemberListByGroupIDMerlin(req.GroupID)
		if err != nil {
		...
		for _, v := range memberList {
			...
			if groupInfo.GroupType == constant.WorkingGroup {
				c.ConversationID = utils.GetConversationIDBySessionType(req.GroupID, constant.SuperGroupChatType)
				c.ConversationType = constant.SuperGroupChatType
			} else {
		...
		rocksCache.DelSpaceGroupInfoFromCache(req.OpUserID)
...
// CreateSysUserGroup 创建官方群组
func (s *groupServer) CreateSysUserGroup(ctx context.Context, req *pbGroup.CreateSysUserGroupReq) (*pbGroup.CreateSysUserGroupRes, error) {
	resp := &pbGroup.CreateSysUserGroupRes{
		CommonResp: &pbGroup.CommonResp{
			ErrCode: 0,
			ErrMsg:  "",
		},
	...
	if len(req.UserId) == 0 {
		log.NewError("", "CreateSysUserGroup userIds len is zero ")
		return &pbGroup.CreateSysUserGroupRes{CommonResp: &pbGroup.CommonResp{ErrCode: constant.ErrDB.ErrCode, ErrMsg: "sysUserArray length is 0"}}, nil
	}

	err := imdb.CreateSysUserGroup(req.UserId)
	if err != nil {
		log.NewError("", "CreateSysUserGroup failed ", err.Error())
		return &pbGroup.CreateSysUserGroupRes{CommonResp: &pbGroup.CommonResp{ErrCode: constant.ErrDB.ErrCode, ErrMsg: constant.ErrDB.ErrMsg}}, nil
	}

	//config.Config.InitUser.UserId

	return resp, nil
}
func (s *groupServer) CreateCommunity(ctx context.Context, req *pbGroup.CreateCommunityReq) (*pbGroup.CreateCommunityResp, error) {
	//TODO implement me
	log.NewInfo(req.OperationID, "CreateGroup, args ", req.String())
	if !token_verify.CheckAccess(req.OpUserID, req.OwnerUserID) {
		log.NewError(req.OperationID, "CheckAccess false ", req.OpUserID, req.OwnerUserID)
		return &pbGroup.CreateCommunityResp{ErrCode: constant.ErrAccess.ErrCode,
			ErrMsg: constant.ErrAccess.ErrMsg}, nil
	}
	if req.GroupInfo.IsFees == 0 {
		if err, count := imdb.GetFreeGroupCount(req.OpUserID); err != nil || count >= config.Config.FreeGroupCount {
			return &pbGroup.CreateCommunityResp{ErrCode: constant.ErrAccess.ErrCode,
				ErrMsg: "can't create new community ，fee count over"}, nil
		}
	} else {
		log.Error("检查付费订单是否完成付费")
	}
	...
	dbUserInfo, _ := rocksCache.GetUserBaseInfoFromCache(req.OpUserID)
	//to group
	groupInfo := db.Group{}
	utils.CopyStructFields(&groupInfo, req.GroupInfo)
	groupInfo.CreatorUserID = req.OpUserID
	...
	groupInfo.CreateTime = time.Now()
	if dbUserInfo != nil {
		groupInfo.Introduction = dbUserInfo.UserIntroduction
	}
	if groupInfo.NotificationUpdateTime.Unix() < 0 {
		groupInfo.NotificationUpdateTime = utils.UnixSecondToTime(0)
	}
	//直接进入社区
	groupInfo.NeedVerification = constant.Directly
	err := imdb.InsertIntoGroup(groupInfo)
	if err != nil {
		log.NewError(req.OperationID, "InsertIntoGroup failed, ", err.Error(), groupInfo)
		return &pbGroup.CreateCommunityResp{ErrCode: constant.ErrDB.ErrCode, ErrMsg: constant.ErrDB.ErrMsg}, nil
	}
	var okUserIDList []string
	resp := &pbGroup.CreateCommunityResp{GroupInfo: &open_im_sdk.GroupInfo{}}
	groupMember := db.GroupMember{}
	us := &db.User{}
	if req.OwnerUserID != "" {
		us, err = imdb.GetUserByUserID(req.OwnerUserID)
		...
		//to group member
		groupMember = db.GroupMember{GroupID: groupId, RoleLevel: constant.GroupOwner,
			OperatorUserID: req.OpUserID, JoinSource: constant.JoinByInvitation, InviterUserID: req.OpUserID}
		utils.CopyStructFields(&groupMember, us)
		err = imdb.InsertIntoGroupMember(groupMember)
		if err != nil {
			log.NewError(req.OperationID, "InsertIntoGroupMember failed ", err.Error(), groupMember)
	...
	//创建两个默认的
	var inputNewChannel []*db.GroupChannel
	inputNewChannel = append(inputNewChannel, &db.GroupChannel{
		CreatedAt:   time.Now(),
		UpdatedAt:   time.Now(),
		GroupID:     groupInfo.GroupID,
		ChannelID:   "1",
		ChannelName: "Notice Channel", //公告频道
		ChannelType: "0",              //公告频道
	})
	inputNewChannel = append(inputNewChannel, &db.GroupChannel{
		CreatedAt:   time.Now(),
		...
		GroupID:     groupInfo.GroupID,
		ChannelID:   "2",
		ChannelName: "Public Channel", //公告频道
		ChannelType: "1",              //群众频道
	})
	imdb.InsertIntoGroupChannel(&inputNewChannel)
	if len(okUserIDList) != 0 {
		log.NewInfo(req.OperationID, "rpc CreateGroup return ", resp.String())
		if req.GroupInfo.GroupType != constant.SuperGroup {
			for _, userID := range okUserIDList {
...
func (s *groupServer) UpdateCommunityChannel(ctx context.Context, req *pbGroup.CommunityChannelReq) (*pbGroup.CommunityChannelResp, error) {
	log.NewInfo(req.OperationID, utils.GetSelfFuncName(), "rpc args ", req.String())
	if !hasGroupChannelAccess(req.ChannelInfo.GroupID, req.OpUserID, req.OperationID) {
		//判断是否有权限
		log.NewError(req.OperationID, "no access ", req)
		return &pbGroup.CommunityChannelResp{CommonResp: &pbGroup.CommonResp{ErrCode: constant.ErrAccess.ErrCode, ErrMsg: constant.ErrAccess.ErrMsg}}, nil
	}
	if req.OpInfo != "add" && !imdb.IsGroupChannel(req.ChannelInfo.GroupID,
		req.ChannelInfo.ChannelID) {
		return &pbGroup.CommunityChannelResp{CommonResp: &pbGroup.CommonResp{ErrCode: constant.ErrInternal.ErrCode, ErrMsg: constant.ErrInternal.ErrMsg}}, nil
	}
	if req.OpInfo == "update" {
		channelInfo, err := imdb.GetGroupChannelInfo(req.ChannelInfo.GroupID,
			req.ChannelInfo.ChannelID)
		if err != nil || channelInfo.ChannelStatus == constant.GroupStatusDismissed {
			errMsg := " community channel status is dismissed "
			return &pbGroup.CommunityChannelResp{CommonResp: &pbGroup.CommonResp{ErrCode: constant.ErrStatus.ErrCode, ErrMsg: errMsg}}, nil
		}
	}
	if req.OpInfo == "del" {
		channelInfo, err := imdb.GetGroupChannelInfo(req.ChannelInfo.GroupID,
			req.ChannelInfo.ChannelID)
		if err != nil || channelInfo.ChannelStatus == constant.GroupStatusDismissed {
			errMsg := " community channel status has dismissed "
			return &pbGroup.CommunityChannelResp{CommonResp: &pbGroup.CommonResp{ErrCode: constant.ErrStatus.ErrCode, ErrMsg: errMsg}}, nil
		}
	}
	var err error
	switch req.OpInfo {
	case "add":
		channelidstr := utils.Md5(req.ChannelInfo.GroupID + strconv.FormatInt(time.Now().UnixNano(), 10))
		bi := big.NewInt(0)
		bi.SetString(channelidstr[0:8], 16)
		channelid := bi.String()
		//创建两个默认的
		var inputNewChannel []*db.GroupChannel
		channelType := "1"
		if req.ChannelInfo.ChannelType != "" {
			channelType = req.ChannelInfo.ChannelType
		}
		req.ChannelInfo.ChannelID = channelid
		inputNewChannel = append(inputNewChannel, &db.GroupChannel{
			CreatedAt:   time.Now(),
			UpdatedAt:   time.Now(),
			GroupID:     req.ChannelInfo.GroupID,
			ChannelID:   channelid,
			ChannelName: req.ChannelInfo.ChannelName, //公告频道
			ChannelType: channelType,                 //群众频道类型
		})
		err = imdb.InsertIntoGroupChannel(&inputNewChannel)
	case "del":
		err = imdb.DissGroupChannel(req.ChannelInfo.GroupID, req.ChannelInfo.ChannelID)
	case "update":
		newGroupChannel := &db.GroupChannel{
			CreatedAt:   time.Now(),
			UpdatedAt:   time.Now(),
			GroupID:     req.ChannelInfo.GroupID,
			ChannelID:   req.ChannelInfo.ChannelID,
			ChannelName: req.ChannelInfo.ChannelName, //公告频道
		}
		err = imdb.UpdateGroupChannel(newGroupChannel)
	}
	if err != nil {
		log.NewError(req.OperationID, "OperateGroupStatus failed ", req.ChannelInfo.GroupID, constant.GroupStatusDismissed)
		return &pbGroup.CommunityChannelResp{CommonResp: &pbGroup.CommonResp{ErrCode: constant.ErrDB.ErrCode, ErrMsg: constant.ErrDB.ErrMsg}}, nil
	}
	//推送channel 更新通知
	chat.GroupChannelNotification(req.OperationID,
		req.OpUserID,
		req.ChannelInfo.GroupID,
	...
	//		errMsg := req.OperationID + "getcdv3.GetDefaultConn == nil"
	//		log.NewError(req.OperationID, errMsg)
	//		return &pbGroup.CommunityChannelResp{CommonResp: &pbGroup.CommonResp{ErrCode: constant.ErrInternal.ErrCode, ErrMsg: errMsg}}, nil
	//	}
	//	client := pbCache.NewCacheClient(etcdConn)
	//	cacheResp, err := client.GetGroupMemberIDListFromCache(context.Background(), getGroupMemberIDListFromCacheReq)
	//	if err != nil {
	//		log.NewError(req.OperationID, "GetGroupMemberIDListFromCache rpc call failed ", err.Error())
	//		return &pbGroup.CommunityChannelResp{CommonResp: &pbGroup.CommonResp{}}, nil
	//	}
	//	if cacheResp.CommonResp.ErrCode != 0 {
	//		log.NewError(req.OperationID, "GetGroupMemberIDListFromCache rpc logic call failed ", cacheResp.String())
	//		return &pbGroup.CommunityChannelResp{CommonResp: &pbGroup.CommonResp{}}, nil
	...
	//	conversation := pbConversation.Conversation{
	//		OwnerUserID: req.OpUserID,
	//		ConversationID: utils.GetConversationIDBySessionType(req.ChannelInfo.GroupId,
...
func (s *groupServer) GetCommunityAllChannel(ctx context.Context, req *pbGroup.CommunityChannelAllListReq) (*pbGroup.CommunityChannelAllListResp, error) {
	log.NewInfo(req.OperationID, "GetGroupAllMember, args ", req.String())
	var resp pbGroup.CommunityChannelAllListResp
	resp.CommonResp = &pbGroup.CommonResp{}
	groupInfo, err := rocksCache.GetGroupInfoFromCache(req.GroupID)
	if err != nil {
		log.NewError(req.OperationID, utils.GetSelfFuncName(), err.Error(), req.GroupID)
	...
	if groupInfo.GroupType != constant.SuperGroup {
		channelList, err := imdb.GetGroupChannelAllInfo(req.GroupID)
		if err != nil {
			log.NewError(req.OperationID, utils.GetSelfFuncName(), err.Error(), req.GroupID)
			resp.CommonResp.ErrCode = constant.ErrDB.ErrCode
			resp.CommonResp.ErrMsg = constant.ErrDB.ErrMsg
		...
		for _, v := range channelList {
			var node open_im_sdk.GroupChannelInfo
			cp.GroupChannelDBCopyOpenIM(&node, v)
			resp.ChannelList = append(resp.ChannelList, &node)
		}
	}
	log.NewInfo(req.OperationID, "GetGroupAllChannel rpc return ", len(resp.ChannelList))
	return &resp, nil
}

func (s *groupServer) GetHotSpace(ctx context.Context, req *pbGroup.GetHotSpaceReq) (*pbGroup.GetHotSpaceResp, error) {
	resp := pbGroup.GetHotSpaceResp{
		CommonResp: &pbGroup.CommonResp{},
	}
	spaces, err := imdb.GetHotSpace(req.UserId, int(req.PageIndex), int(req.PageSize))
	if err != nil {
		log.NewError(req.OperationID, utils.GetSelfFuncName(), err.Error())
		resp.CommonResp.ErrCode = constant.ErrDB.ErrCode
		resp.CommonResp.ErrMsg = constant.ErrDB.ErrMsg
	...
	for _, v := range spaces {
		node := pbGroup.PublicSpaceUserInfo{
			Group:       &open_im_sdk.GroupInfo{},
			FollowCount: v.FollowCount,
			IsFollow:    v.IsFollow,
		}
		// 获取用户信息
		userInfo, err := rocksCache.GetUserBaseInfoFromCache(v.UserId)
		if err != nil {
			log.NewError(req.OperationID, utils.GetSelfFuncName(), err.Error())
			continue
		}
		utils.CopyStructFields(&node, userInfo)
		// 获取群组信息
		groupInfo, err := rocksCache.GetSpaceInfoByUser(v.UserId)
		if err != nil {
			log.NewError(req.OperationID, utils.GetSelfFuncName(), err.Error())
			continue
		}
		utils.CopyStructFields(&node.Group, groupInfo)
		resp.UserInfoList = append(resp.UserInfoList, &node)
	}

	return &resp, nil
}
func (s *groupServer) GetMyFollowingSpace(ctx context.Context, req *pbGroup.GetHotSpaceReq) (*pbGroup.GetHotSpaceResp, error) {
	resp := pbGroup.GetHotSpaceResp{
		CommonResp: &pbGroup.CommonResp{},
	}
	spaces, err := imdb.GetMyFollowingSpace(req.UserId, int(req.PageIndex), int(req.PageSize))
	if err != nil {
		log.NewError(req.OperationID, utils.GetSelfFuncName(), err.Error())
		resp.CommonResp.ErrCode = constant.ErrDB.ErrCode
		resp.CommonResp.ErrMsg = constant.ErrDB.ErrMsg
...
func (s *groupServer) SearchCommunity(ctx context.Context, req *pbGroup.SearchCommunityReq) (*pbGroup.SearchCommunityResp, error) {
	log.NewInfo(req.OperationID, "SearchCommunity, args ", req.String())
	resp := new(pbGroup.SearchCommunityResp)
	dbFindGroup, err := imdb.SearchCommunityInfo(req.SearchName)
	if err != nil {
		resp.ErrCode = constant.ErrDB.ErrCode
		resp.ErrMsg = err.Error()
		return resp, nil
	}
	for _, value := range dbFindGroup {
		newGroupInfo := &pbGroup.HotGroupInfo{
			GroupID:       value.GroupID,
			GroupName:     value.GroupName,
			Notification:  value.Notification,
			Introduction:  value.Introduction,
			FaceURL:       value.FaceURL,
			OwnerUserID:   value.CreatorUserID,
			CreateTime:    value.CreateTime.Unix(),
			CreatorUserID: value.CreatorUserID,
			GroupType:     uint32(value.GroupType),
			IsJoinEd:      false,
		}
		dbUser, err := imdb.GetUserByUserID(newGroupInfo.CreatorUserID)
		if err == nil {
			newGroupInfo.CreatorFaceURL = dbUser.FaceURL
		}
		int64Count, err := imdb.GetGroupMembersCount(newGroupInfo.GroupID, "")
		if err == nil {
			newGroupInfo.MemberCount = uint32(int64Count)
		}
		newGroupInfo.IsJoinEd = imdb.IsExistGroupMember(value.GroupID, req.OpUserID) //是否是成员
		resp.GroupInfo = append(resp.GroupInfo, newGroupInfo)
	}
	return resp, nil
}

func (s *groupServer) GetCommunityChannelByGroupIDAndChannelID(ctx context.Context, req *pbGroup.CommunityChannelInfoReq) (*pbGroup.CommunityChannelInfoResp, error) {
	log.NewInfo(req.OperationID, "GetGroupAllMember, args ", req.String())
	var resp pbGroup.CommunityChannelInfoResp
	resp.CommonResp = &pbGroup.CommonResp{}
	groupInfo, err := rocksCache.GetGroupInfoFromCache(req.GroupID)
	if err != nil {
		log.NewError(req.OperationID, utils.GetSelfFuncName(), err.Error(), req.GroupID)
	...
	if groupInfo.GroupType != constant.SuperGroup {
		channelInfo, err := imdb.GetGroupChannelByGroupIDAndChannelID(req.GroupID, req.ChannelID)
		if err != nil {
			log.NewError(req.OperationID, utils.GetSelfFuncName(), err.Error(), req.GroupID)
			resp.CommonResp.ErrCode = constant.ErrDB.ErrCode
			resp.CommonResp.ErrMsg = constant.ErrDB.ErrMsg
		...
		resp.ChannelList = new(open_im_sdk.GroupChannelInfo)
		utils.CopyStructFields(resp.ChannelList, channelInfo)
	}
	log.NewInfo(req.OperationID, "GetGroupAllChannel rpc return ", resp.ChannelList)
	return &resp, nil
}
func (s *groupServer) GetGroupMemberHaveNftCount(ctx context.Context, req *pbGroup.GetGroupHaveNftMemberCountReq) (*pbGroup.GetGroupHaveNftMemberCountResp, error) {
	resp := new(pbGroup.GetGroupHaveNftMemberCountResp)
	groupID := req.GroupID
	opUserID := req.OpUserID
	cacheResp, err2 := rocksCache.GetGroupMemberIDListFromCache(groupID)
	if err2 == nil {
		if !utils.IsContain(opUserID, cacheResp) {
			resp.CommonResp = &pbGroup.CommonResp{ErrCode: constant.ErrInternal.ErrCode, ErrMsg: "不是群成员无法查看群内有多少个人"}
			return resp, nil
		}
		resp.CommonResp = &pbGroup.CommonResp{}
		resp.Count, err2 = imdb.GetGroupHaveNftMemberCount(groupID)
		if err2 == nil {
			return resp, nil
		}
	}
	resp.CommonResp = &pbGroup.CommonResp{ErrCode: constant.ErrInternal.ErrCode, ErrMsg: err2.Error()}
	return resp, nil
}

const TAEGETMEMBERCOUNT = 100

// 1 群主
// 先查询当月是否已经领取了群主100人的奖励的积分。
// 如果已经领取 则返回不可以再领取： 若一个群有100人，另外一个群是200 人，先领取先拿
// 否则可以领取
// 2 普通成员：
// 查看当月本群是否领取过奖励
// 如果已经领取则不可以在领取
// 如果没有领取 那么提示可以领取。
func (s *groupServer) IsCanGetGroupMemberHaveNftCountReword(ctx context.Context, req *pbGroup.GetGroupHaveNftMemberCountReq) (*pbGroup.GetGroupHaveNftMemberCountResp, error) {
	//获取本群的创建的时间和状态
	resp := new(pbGroup.GetGroupHaveNftMemberCountResp)
	dbGroupInfo, err := rocksCache.GetGroupInfoFromCache(req.GroupID)
	if err != nil {
		resp.CommonResp = &pbGroup.CommonResp{ErrCode: constant.ErrInternal.ErrCode, ErrMsg: err.Error()}
		return resp, nil
	}
	//群的状态正常
	if dbGroupInfo.Status == 2 {
		resp.CommonResp = &pbGroup.CommonResp{ErrCode: constant.ErrInternal.ErrCode, ErrMsg: "group had dismiss"}
		return resp, nil
	}
	timeNow := time.Now()
	fmt.Println("timeNow.Sub(dbGroupInfo.CreateTime)>>>>>>", dbGroupInfo.CreateTime,
		dbGroupInfo.GroupID, timeNow.Sub(dbGroupInfo.CreateTime), timeNow.Sub(dbGroupInfo.CreateTime).Minutes())
	//群的创建时间次月
	if config.Config.IsPublicEnv {
		if timeNow.Before(dbGroupInfo.CreateTime) || timeNow.Sub(dbGroupInfo.CreateTime).Hours()/24 < 30 {
			resp.CommonResp = &pbGroup.CommonResp{ErrCode: constant.ErrInternal.ErrCode, ErrMsg: "reword will get in next mouth"}
			return resp, nil
		}
	} else {
		if timeNow.Before(dbGroupInfo.CreateTime) || timeNow.Sub(dbGroupInfo.CreateTime).Minutes() < 10 {
			resp.CommonResp = &pbGroup.CommonResp{ErrCode: constant.ErrInternal.ErrCode, ErrMsg: "reword will get in next mouth"}
			return resp, nil
		}
	}

	userInfoGroup, err := rocksCache.GetUserBaseInfoFromCache(dbGroupInfo.CreatorUserID) //群主自己必须携带nft
	if userInfoGroup.TokenContractChain == "" {
		resp.CommonResp = &pbGroup.CommonResp{ErrCode: constant.ErrInternal.ErrCode, ErrMsg: "this group owner not have nft"}
		return resp, nil
	}
	memberList, err2 := rocksCache.GetGroupMemberIDListFromCache(req.GroupID)
	if err2 != nil {
		resp.CommonResp = &pbGroup.CommonResp{ErrCode: constant.ErrInternal.ErrCode, ErrMsg: err.Error()}
		return resp, nil
	}
	if !utils.IsContain(req.OpUserID, memberList) {
		resp.CommonResp = &pbGroup.CommonResp{ErrCode: constant.ErrInternal.ErrCode, ErrMsg: "your not in this community"}
		return resp, nil
	}
	targetMemberCount := TAEGETMEMBERCOUNT
	if !config.Config.IsPublicEnv {
		targetMemberCount = 3
	}
	if CountUserIDList, err2 := imdb.GetGroupHaveNftMemberIDListCount(req.GroupID); err2 != nil || len(CountUserIDList) < targetMemberCount {
		resp.CommonResp = &pbGroup.CommonResp{ErrCode: constant.ErrInternal.ErrCode, ErrMsg: "not enough member count have nft"}
		return resp, nil
	} else if !utils.IsContain(req.OpUserID, CountUserIDList) {
		resp.CommonResp = &pbGroup.CommonResp{ErrCode: constant.ErrInternal.ErrCode, ErrMsg: "your not have nft"}
		return resp, nil
	}
	resp.Count = 0
	if req.OpUserID == dbGroupInfo.CreatorUserID {
		//如果是群主  判断当月是否已经领取
		if imdb.IsQunZhuHadGetRewordMemberCount(req.OpUserID, "12", timeNow, req.GroupID) {
			resp.CommonResp = &pbGroup.CommonResp{}
			resp.Count = 1
			return resp, nil
		} else {
		...
		// 如果是普通的成员
		if imdb.IsQunZhuHadGetRewordMemberCount(req.OpUserID, "13", timeNow, req.GroupID) {
			resp.CommonResp = &pbGroup.CommonResp{}
			resp.Count = 1
			return resp, nil
		} else {
...
type CheckIDOResponseDataResp struct {
	Code    int    `json:"code"`
...
func checkisendidoproject(ctx context.Context, groupid, userid string) (bool, error) {
	fmturl := fmt.Sprintf("%s/idoApi/getIDOs?id=%s&owner=%s",
		config.Config.UserGroupIdoBackGround,
		groupid,
		userid)
	fmt.Println(fmturl)
	resuldatabyte, err := utils.HttpGet(fmturl)
	if err != nil || len(resuldatabyte) == 0 {
		return false, errors.New("post data is empty")
	}
	var postdataresult CheckIDOResponseDataResp
	err3 := json.Unmarshal(resuldatabyte, &postdataresult)

	fmt.Println("resuldatabyte>>>>>>", string(resuldatabyte))
	if err3 != nil {
		return false, err
	} else {
		if len(postdataresult.Data) == 0 {
			return true, nil
		} else {
			for _, value := range postdataresult.Data {
				if len(value.Data.TimeList) > 0 {
					if value.Data.TimeList[len(value.Data.TimeList)-1] > time.Now().Unix() {
						return false, nil
					}
				}
			}
...
func (s *groupServer) ColligateSearch(ctx context.Context, req *pbGroup.ColligateSearchReq) (*pbGroup.ColligateSearchResp, error) {
	resp := &pbGroup.ColligateSearchResp{
		CommonResp: &pbGroup.CommonResp{},
	}
	if req.SearchKey == "" {
		resp.CommonResp = &pbGroup.CommonResp{ErrCode: constant.ErrInternal.ErrCode, ErrMsg: "search key is empty"}
		return resp, nil
	}
	searchBody, err := imdb.ColligateSearch(req.SearchKey)
	if err != nil {
		resp.CommonResp = &pbGroup.CommonResp{ErrCode: constant.ErrInternal.ErrCode, ErrMsg: err.Error()}
		return resp, nil
	}
	resp.ColligateSearchBody = searchBody
	return resp, nil
}
...
func FollowAddFriend(operationID, formUserId, toUserId string) error {
	...
	log.NewInfo(operationID, "FollowAddFriend success", formUserId, toUserId)
	return nil
}
