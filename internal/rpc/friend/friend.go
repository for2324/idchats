...
func (s *friendServer) AddFriendResponse(ctx context.Context, req *pbFriend.AddFriendResponseReq) (*pbFriend.AddFriendResponseResp, error) {
	...
	if req.HandleResult == constant.FriendFlag {
		...
		if err == nil {
			...
			//Establish two single friendship
			toInsertFollow := db.Friend{
				OwnerUserID:    req.CommID.FromUserID,
				FriendUserID:   req.CommID.ToUserID,
				OperatorUserID: req.CommID.OpUserID}
...
func (s *friendServer) GetBlacklist(ctx context.Context, req *pbFriend.GetBlacklistReq) (*pbFriend.GetBlacklistResp, error) {
	...
	for _, userID := range blackIDList {
		user, err := rocksCache.GetUserBaseInfoFromCache(userID)
		if err != nil {
...
func (s *friendServer) SetFriendRemark(ctx context.Context, req *pbFriend.SetFriendRemarkReq) (*pbFriend.SetFriendRemarkResp, error) {
	...
	err := imdb.CheckIsFriendCommentInsert(req.CommID.FromUserID, req.CommID.ToUserID, req.Remark)
	if err != nil {
...
func (s *friendServer) GetFriendList(ctx context.Context, req *pbFriend.GetFriendListReq) (*pbFriend.GetFriendListResp, error) {
	...
	useropinfo, err := rocksCache.GetUserBaseInfoFromCache(req.CommID.OpUserID)
	...
	for _, friendUser := range friendList {
		...
		cp.FriendDBCopyOpenIMByChainID(&friendUserInfo, friendUser, utils.Int32ToString(useropinfo.Chainid))
...
// //
// }
func (s *friendServer) GetFriendFollowList(ctx context.Context, req *pbFriend.GetFriendFollowListReq) (*pbFriend.GetFriendFollowListResp, error) {
	log.NewInfo(req.CommID.OperationID, utils.GetSelfFuncName(), " args ", req.String())
	if !token_verify.CheckAccess(req.CommID.OpUserID, req.CommID.FromUserID) {
		log.NewError(req.CommID.OperationID, "CheckAccess false ", req.CommID.OpUserID, req.CommID.FromUserID)
		return &pbFriend.GetFriendFollowListResp{CommonResp: &pbFriend.CommonResp{
			ErrCode: constant.ErrAccess.ErrCode, ErrMsg: constant.ErrAccess.ErrMsg,
		}}, nil
	}
	resultuser, err := imdb.GetFollowListByFollowAndUserID(req.CommID.ToUserID, req.IsFollow)
	if err != nil {
		log.NewError(req.CommID.OperationID, "GetFriendFollowList failed ", err.Error(), req.CommID.FromUserID)
		return &pbFriend.GetFriendFollowListResp{CommonResp: &pbFriend.CommonResp{ErrCode: constant.ErrDB.ErrCode, ErrMsg: constant.ErrDB.ErrMsg}}, nil
	}
	log.NewDebug(req.CommID.OperationID, utils.GetSelfFuncName(), "GetFollowListByFollowAndUserID::>>>>", utils.StructToJsonString(resultuser))
	var selectChainID int32
	if req.CommID.OpUserID != "" {
		useropinfo, _ := rocksCache.GetUserBaseInfoFromCache(req.CommID.OpUserID)
		selectChainID = useropinfo.Chainid
	} else {
		selectChainID = 1
	}

	var userInfoList []*sdkws.PublicUserInfo
	for _, value := range resultuser {
		tempdate := new(sdkws.PublicUserInfo)
		cp.UserDBCopyOpenIMPublicUser(tempdate, value)
		ensNameUser, _ := imdb.GetUserEnsNameByChainID(tempdate.UserID, utils.Int32ToString(selectChainID))
		userInfoTemp, _ := imdb.GetUserByUserID(tempdate.UserID)
		if ensNameUser != "" {
			tempdate.Nickname = ensNameUser
		}
		tempdate.FaceURL = userInfoTemp.FaceURL
		userInfoList = append(userInfoList, tempdate)
	}
	log.NewDebug(req.CommID.OperationID, "GetFriendFollowList userInfoList::", utils.StructToJsonString(userInfoList))
	return &pbFriend.GetFriendFollowListResp{
		CommonResp:     &pbFriend.CommonResp{},
		PublicUserInfo: userInfoList,
	}, nil
}

func (s *friendServer) IsFollowEachOther(ctx context.Context, req *pbFriend.IsFriendReq) (*pbFriend.IsFriendResp, error) {
	log.NewInfo(req.CommID.OperationID, req.String())
	if !token_verify.CheckAccess(req.CommID.OpUserID, req.CommID.FromUserID) {
		log.NewError(req.CommID.OperationID, "CheckAccess false ", req.CommID.OpUserID, req.CommID.FromUserID)
		return &pbFriend.IsFriendResp{ErrCode: constant.ErrAccess.ErrCode, ErrMsg: constant.ErrAccess.ErrMsg}, nil
	}
	friendIDList, err := rocksCache.GetFollowEachOtherFromCache(req.CommID.FromUserID)
	if err != nil {
		log.NewError(req.CommID.OperationID, utils.GetSelfFuncName(), err.Error(), req.CommID.FromUserID)
		return &pbFriend.IsFriendResp{ErrMsg: err.Error(), ErrCode: constant.ErrDB.ErrCode}, nil
	}
...
func (s *friendServer) FollowAddFriend(ctx context.Context, req *pbFriend.FollowAddFriendReq) (*pbFriend.FollowAddFriendResp, error) {
	log.NewInfo(req.CommID.OperationID, "UpdateFollowFriendApplication args ", req.String())
	ok := token_verify.CheckAccess(req.CommID.OpUserID, req.CommID.FromUserID)
	if !ok {
		log.NewError(req.CommID.OperationID, "CheckAccess false ", req.CommID.OpUserID, req.CommID.FromUserID)
		return &pbFriend.FollowAddFriendResp{CommonResp: &pbFriend.CommonResp{ErrCode: constant.ErrAccess.ErrCode, ErrMsg: constant.ErrAccess.ErrMsg}}, nil
	}
	if req.CommID.FromUserID == req.CommID.ToUserID {
		log.NewError(req.CommID.OperationID, "CheckAccess false ", req.CommID.OpUserID, req.CommID.FromUserID)
		return &pbFriend.FollowAddFriendResp{CommonResp: &pbFriend.CommonResp{ErrCode: constant.ErrArgs.ErrCode,
			ErrMsg: "don't follow yourself"}}, nil
	}
	var followflag int8
	if req.Follow {
		followflag = 1
	...
	friendRequest := db.UserFollow{
		HandleResult: 0,
		Follow:       followflag,
		CreateTime:   time.Now(),
		HandleTime:   time.Now(),
		FollowUserID: req.CommID.ToUserID}
	utils.CopyStructFields(&friendRequest, req.CommID)
	// {openIM001 openIM002 0 test add friend 0001-01-01 00:00:00 +0000 UTC   0001-01-01 00:00:00 +0000 UTC }]
	fmt.Println(req.CommID.OperationID, "UpdateFollowFriendApplication args ", friendRequest)

	err := imdb.FollowInsertFriendApplication(&friendRequest,
		map[string]interface{}{"handle_result": 0, "follow": friendRequest.Follow, "create_time": friendRequest.CreateTime,
			"handler_user_id": "", "handle_msg": "", "handle_time": utils.UnixSecondToTime(0), "ex": ""})
	if err != nil {
		fmt.Println(req.CommID.OperationID, "FollowAddFriend failed ", err.Error(), friendRequest)
		return &pbFriend.FollowAddFriendResp{CommonResp: &pbFriend.CommonResp{ErrCode: constant.ErrDB.ErrCode, ErrMsg: constant.ErrDB.ErrMsg}}, nil
	} else {
		fmt.Println(req.CommID.OperationID, "FollowAddFriend ok ", utils.StructToJsonString(friendRequest))
	}
	Userptr, err := imdb.GetUserByUserID(friendRequest.FollowUserID)
	if err != nil {
		fmt.Println(req.CommID.OperationID, "FollowAddFriend failed ", err.Error(), friendRequest)
		return &pbFriend.FollowAddFriendResp{CommonResp: &pbFriend.CommonResp{ErrCode: constant.ErrDB.ErrCode, ErrMsg: constant.ErrDB.ErrMsg}}, nil
	}
	var userInfoList []*sdkws.PublicUserInfo
	tempdate := new(sdkws.PublicUserInfo)
	cp.UserDBCopyOpenIMPublicUser(tempdate, Userptr)
	userInfoList = append(userInfoList, tempdate)

	if req.Follow {
		chat.FriendFollowApplicationNotification(req)
	} else {
		chat.FriendFollowDeleteApplicationNotification(req)
	}
	//更新Follow 缓存
	delFriendIDListFromCacheReq := &pbCache.DelFollowEachOtherFriendIDListFromCacheReq{OperationID: req.CommID.OperationID}
	etcdConn := getcdv3.GetDefaultConn(config.Config.Etcd.EtcdSchema, strings.Join(config.Config.Etcd.EtcdAddr, ","), config.Config.RpcRegisterName.OpenImCacheName, req.CommID.OperationID)
	if etcdConn == nil {
		errMsg := req.CommID.OperationID + "getcdv3.GetDefaultConn == nil"
		log.NewError(req.CommID.OperationID, errMsg)
		return &pbFriend.FollowAddFriendResp{CommonResp: &pbFriend.CommonResp{ErrCode: constant.ErrInternal.ErrCode, ErrMsg: errMsg}}, nil
	}
	client := pbCache.NewCacheClient(etcdConn)
	delFriendIDListFromCacheReq.UserID = req.CommID.ToUserID
	respPb, err := client.DelFollowEachOtherFriendIDListFromCache(context.Background(), delFriendIDListFromCacheReq)
	if err != nil {
		log.NewError(req.CommID.OperationID, utils.GetSelfFuncName(), "DelFollowEachOtherFriendIDListFromCache failed", err.Error())
		return &pbFriend.FollowAddFriendResp{CommonResp: &pbFriend.CommonResp{ErrCode: constant.ErrServer.ErrCode, ErrMsg: constant.ErrServer.ErrMsg}}, nil
	}
	if respPb.CommonResp.ErrCode != 0 {
		log.NewError(req.CommID.OperationID, utils.GetSelfFuncName(), "DelFollowEachOtherFriendIDListFromCache failed", respPb.CommonResp.String())
		return &pbFriend.FollowAddFriendResp{CommonResp: &pbFriend.CommonResp{ErrCode: respPb.CommonResp.ErrCode, ErrMsg: respPb.CommonResp.ErrMsg}}, nil
	}
	delFriendIDListFromCacheReq.UserID = req.CommID.FromUserID
	respPb, err = client.DelFollowEachOtherFriendIDListFromCache(context.Background(), delFriendIDListFromCacheReq)
	if err != nil {
	...
	// 查询备注关系
	friendsList, err := imdb.SelectFriendsListByUserId(req.CommID.ToUserID)
	if err != nil {
		return nil, err
	}
	friendIdAndRemarkMap := make(map[string]string)
	for _, v := range friendsList {
		friendIdAndRemarkMap[v.FriendUserID] = v.Remark
	}
	// 备注关系，添加到返回结果
	for i, v := range userInfoList {
		userInfoList[i].Remark = friendIdAndRemarkMap[v.UserID]
	}
	resultFollowResp := &pbFriend.FollowAddFriendResp{
		CommonResp:     &pbFriend.CommonResp{},
		PublicUserInfo: userInfoList,
	}
	//add to friend list
	if req.Follow {
		...
		chat.FriendDeletedNotification(&pbFriend.DeleteFriendReq{
			CommID: req.CommID,
		})
	}
	return resultFollowResp, nil
...
func (s *friendServer) GetUserFollowingCount(ctx context.Context, req *pbFriend.GetUerFollowingCountReq) (*pbFriend.GetUerFollowingCountResp, error) {
	log.NewInfo(req.OperationID, "GetUserFollowingCount rpc received", req.String())
	count, err := imdb.GetUserFollowingCount(req.UserId)
	if err != nil {
		log.NewError(req.OperationID, "GetUserFollowingCount failed", err.Error(), req.UserId)
		return &pbFriend.GetUerFollowingCountResp{
			CommonResp: &pbFriend.CommonResp{
				ErrCode: 500,
				ErrMsg:  err.Error(),
			},
...
func (s *friendServer) GetUserFollowedCount(ctx context.Context, req *pbFriend.GetUerFollowedCountReq) (*pbFriend.GetUerFollowedCountResp, error) {
	log.NewInfo(req.OperationID, "GetUserFollowsCount rpc received", req.String())
	count, err := imdb.GetUserFollowsCount(req.UserId)
	if err != nil {
		log.NewError(req.OperationID, "GetUserFollowsCount failed", err.Error(), req.UserId)
		return &pbFriend.GetUerFollowedCountResp{
			CommonResp: &pbFriend.CommonResp{
				ErrCode: 500,
				ErrMsg:  err.Error(),
			},
...
func (s *friendServer) GetUserFollowingList(ctx context.Context, req *pbFriend.GetUserFollowingListReq) (*pbFriend.GetUserFollowingListResp, error) {
	log.NewInfo(req.OperationID, "GetUserFollowingList rpc received", req.String())
	followingList, err := imdb.GetUserFollowingList(req.UserId, req.PageIndex, req.PageSize)
	if err != nil {
		log.NewError(req.OperationID, "GetUserFollowingList failed", err.Error(), req.UserId)
		return &pbFriend.GetUserFollowingListResp{
			CommonResp: &pbFriend.CommonResp{
				ErrCode: 500,
				ErrMsg:  err.Error(),
			},
	...
	var userInfoList []*sdkws.PublicUserInfo
	for _, value := range followingList {
		tempdate := new(sdkws.PublicUserInfo)
		cp.UserDBCopyOpenIMPublicUser(tempdate, value)
		ensNameUser, _ := imdb.GetUserEnsNameByChainID(tempdate.UserID, utils.Int32ToString(1))
		userInfoTemp, _ := imdb.GetUserByUserID(tempdate.UserID)
		if ensNameUser != "" {
			tempdate.Nickname = ensNameUser
		}
...
func (s *friendServer) GetUserFollowedList(ctx context.Context, req *pbFriend.GetUserFollowedListReq) (*pbFriend.GetUserFollowedListResp, error) {
	log.NewInfo(req.OperationID, "GetUserFollowedList rpc received", req.String())
	followedList, err := imdb.GetUserFollowedList(req.UserId, req.PageIndex, req.PageSize)
	if err != nil {
		log.NewError(req.OperationID, "GetUserFollowedList failed", err.Error(), req.UserId)
		return &pbFriend.GetUserFollowedListResp{
			CommonResp: &pbFriend.CommonResp{
				ErrCode: 500,
				ErrMsg:  err.Error(),
			},
...
func (s *friendServer) IsFollowUser(ctx context.Context, req *pbFriend.IsFollowUserReq) (*pbFriend.IsFollowUserResp, error) {
	log.NewInfo(req.OperationID, "IsFollowUser rpc received", req.String())
	isFollow, err := imdb.IsFollowUser(req.FromUserId, req.ToUserId)
	if err != nil {
		log.NewError(req.OperationID, "IsFollowUser failed", err.Error(), req.FromUserId, req.ToUserId)
		return &pbFriend.IsFollowUserResp{
			CommonResp: &pbFriend.CommonResp{
				ErrCode: 500,
				ErrMsg:  err.Error(),
			},
