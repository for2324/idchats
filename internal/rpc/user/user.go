...
func (s *userServer) GetUserInfo(ctx context.Context, req *pbUser.GetUserInfoReq) (*pbUser.GetUserInfoResp, error) {
	...
	requestChainIDUser, _ := rocksCache.GetUserBaseInfoFromCache(req.OpUserID)
	if len(req.UserIDList) > 0 {
		for _, userID := range req.UserIDList {
			var userInfo sdkws.UserInfo
			user, err := rocksCache.GetUserInfoFromCacheByMerLin(userID, utils.Int32ToString(requestChainIDUser.Chainid))
			if err != nil {
...
func (s *userServer) GetSelfUserInfo(ctx context.Context, req *pbUser.GetSelfUserInfoReq) (*pbUser.GetSelfUserInfoResp, error) {
	log.NewInfo(req.OperationID, "GetUserInfo args ", req.String())
	var userInfo sdkws.UserInfo
	user, err := rocksCache.GetUserBaseInfoFromCache(req.UserID)
	if err != nil {
		log.NewError(req.OperationID, "GetUserByUserID failed ", err.Error(), req.UserID)
		return &pbUser.GetSelfUserInfoResp{CommonResp: &pbUser.CommonResp{ErrCode: constant.ErrDB.ErrCode, ErrMsg: err.Error()}}, nil
	}
	utils.CopyStructFields(&userInfo, user)
	userInfo.BirthStr = utils.TimeToString(user.Birth)
	log.NewInfo(req.OperationID, "GetUserInfo rpc return ", userInfo.String())
	return &pbUser.GetSelfUserInfoResp{CommonResp: &pbUser.CommonResp{}, UserInfo: &userInfo}, nil
}
...
func (s *userServer) UpdateUserInfo(ctx context.Context, req *pbUser.UpdateUserInfoReq) (*pbUser.UpdateUserInfoResp, error) {
	...
	fmt.Println("req.userinfo:", utils.StructToJsonString(req.UserInfo), "\n", req.UserInfo.TokenId)
	...
	err = imdb.UpdateUserInfoWithMap(user.UserID, map[string]interface{}{"user_introduction": user.UserIntroduction})
	if err != nil {
	...
	for _, v := range rpcResp.FriendInfoList {
		fmt.Println("\n需要提醒xxxxx 更新我的用户信息", req.OperationID, "UserInfoUpdatedNotification ", req.UserInfo.UserID, v.FriendUser.UserID)
	...
	fmt.Println("推送更新操作", req.UserInfo.UserID, req.OpUserID)
...
func (s *userServer) UpdateUserInfoHead(ctx context.Context, req *pbUser.UpdateUserInfoHeadReq) (*pbUser.UpdateUserInfoHeadResp, error) {
	log.NewInfo(req.OperationID, "UpdateUserInfo args ", req.String())
	if !token_verify.CheckAccess(req.OpUserID, req.UserInfo.UserID) {
		log.NewError(req.OperationID, "CheckAccess false ", req.OpUserID, req.UserInfo.UserID)
		return &pbUser.UpdateUserInfoHeadResp{CommonResp: &pbUser.CommonResp{ErrCode: constant.ErrAccess.ErrCode, ErrMsg: constant.ErrAccess.ErrMsg}}, nil
	}
	oldNickname := ""
	if req.UserInfo.Nickname != "" {
		u, err := imdb.GetUserByUserID(req.UserInfo.UserID)
	...
	var user db.User
	utils.CopyStructFields(&user, req.UserInfo)
	user.TokenContractChain = req.ContractChain
	fmt.Println("req.userinfo:", utils.StructToJsonString(req.UserInfo), "\n", req.UserInfo.TokenId)
	...
	if req.UserInfo.BirthStr != "" {
		...
		if err != nil {
			log.NewError(req.OperationID, "TimeStringToTime failed ", err.Error(), req.UserInfo.BirthStr)
			return &pbUser.UpdateUserInfoHeadResp{CommonResp: &pbUser.CommonResp{ErrCode: constant.ErrArgs.ErrCode, ErrMsg: "TimeStringToTime failed:" + err.Error()}}, nil
		}
		user.Birth = time
	}
	var err error
	if user.TokenContractChain == "" {
		err = imdb.UpdateUserInfoWithMap(user.UserID, map[string]interface{}{"token_contract_chain": "", "face_url": ""})
	} else {
		err = imdb.UpdateUserInfo(user)
	}
	...
	if err != nil {
		log.NewError(req.OperationID, "UpdateUserInfo failed ", err.Error(), user)
		return &pbUser.UpdateUserInfoHeadResp{CommonResp: &pbUser.CommonResp{ErrCode: constant.ErrDB.ErrCode, ErrMsg: constant.ErrDB.ErrMsg}}, nil
	}
	...
	if etcdConn == nil {
		errMsg := req.OperationID + "getcdv3.GetDefaultConn == nil"
		log.NewError(req.OperationID, errMsg)
		return &pbUser.UpdateUserInfoHeadResp{CommonResp: &pbUser.CommonResp{ErrCode: constant.ErrInternal.ErrCode, ErrMsg: errMsg}}, nil
	}
	...
	if err != nil {
		log.NewError(req.OperationID, "GetFriendList failed ", err.Error(), newReq)
		return &pbUser.UpdateUserInfoHeadResp{CommonResp: &pbUser.CommonResp{ErrCode: 500, ErrMsg: err.Error()}}, nil
	}
	for _, v := range rpcResp.FriendInfoList {
		fmt.Println("\n需要提醒xxxxx 更新我的用户信息", req.OperationID, "UserInfoUpdatedNotification ", req.UserInfo.UserID, v.FriendUser.UserID)
		chat.UserInfoUpdatedNotification(req.OperationID, req.UserInfo.UserID, v.FriendUser.UserID)
	...
	if err := rocksCache.DelUserInfoFromCache(user.UserID); err != nil {
		log.NewError(req.OperationID, "userinfo", err.Error())
		return &pbUser.UpdateUserInfoHeadResp{CommonResp: &pbUser.CommonResp{ErrCode: constant.ErrDB.ErrCode, ErrMsg: err.Error()}}, nil
	}
	time.Sleep(1 * time.Second)
	chat.UserInfoUpdatedNotification(req.OperationID, req.UserInfo.UserID, req.OpUserID)
	fmt.Println("推送更新操作", req.UserInfo.UserID, req.OpUserID)
	...
	return &pbUser.UpdateUserInfoHeadResp{CommonResp: &pbUser.CommonResp{}}, nil
}
func (s *userServer) RpcUpdateUserFieldData(ctx context.Context, req *pbUser.SetGlobalRecvMessageOptReq) (*pbUser.SetGlobalRecvMessageOptResp, error) {
	log.NewInfo(req.OperationID, "RpcUpdateUserFieldData args ", req.String())

	var user db.User
	user.UserID = req.UserID
	m := make(map[string]interface{}, 1)
	log.NewDebug(req.OperationID, utils.GetSelfFuncName(), req.GlobalRecvMsgOpt, "set RpcUpdateUserFieldData")
	m[req.FieldName] = req.GlobalRecvMsgOpt
	var err error
	if req.FieldName == "global_recv_msg_opt" {
	...
	} else if req.FieldName == "open_announcement" {
		//记录缓存
		err = db.DB.SetUserGlobalAnnouncementRecvOpt(user.UserID, req.GlobalRecvMsgOpt)
		if err != nil {
			log.NewError(req.OperationID, utils.GetSelfFuncName(), "SetUserGlobalAnnouncementRecvOpt failed ", err.Error(), user)
			return &pbUser.SetGlobalRecvMessageOptResp{CommonResp: &pbUser.CommonResp{ErrCode: constant.ErrDB.ErrCode, ErrMsg: constant.ErrDB.ErrMsg}}, nil
		}
	}
	err = imdb.UpdateUserInfoByMap(user, m)
	if err != nil {
		log.NewError(req.OperationID, "RpcUpdateUserFieldData failed ", err.Error(), user)
		return &pbUser.SetGlobalRecvMessageOptResp{CommonResp: &pbUser.CommonResp{ErrCode: constant.ErrDB.ErrCode, ErrMsg: constant.ErrDB.ErrMsg}}, nil
	}
	if err := rocksCache.DelUserInfoFromCache(user.UserID); err != nil {
...
func (s *userServer) GetUserThird(ctx context.Context, req *pbUser.GetUserThirdInfoReq) (*pbUser.GetUserThirdInfoResp, error) {
	//需要优化， 这边需要改成把所有的查询出来 ，然后判断是否显示还是隐藏， 在内存中判断 尽量不要放在sql里面去做case when的操作
	log.NewInfo(req.OperationID, utils.GetSelfFuncName(), "req: ", req.String())
	resp := &pbUser.GetUserThirdInfoResp{CommonResp: &pbUser.CommonResp{}}
	userlist := req.UserList
	operator := req.OpUserID
	if index := utils.IndexContain(operator, userlist); index != -1 {
		result, err := imdb.GetThirdUserInfo([]string{req.OpUserID}, req.ChainID)
		if err != nil {
			resp.CommonResp.ErrCode = constant.ErrDB.ErrCode
			resp.CommonResp.ErrMsg = err.Error()
			return resp, nil
		}
		log.NewInfo(req.OperationID, utils.GetSelfFuncName(), "resp: ", resp.String(), len(result))
		for _, v := range result {
			pb := &sdkws.UserThirdInfo{
				UserId:       v.UserId,
				ThirdTwitter: v.Twitter,
				EnsDomain:    v.EnsDomain,
				DnsDomain:    v.DnsDomain,
				ShowTwitter:  v.ShowTwitter,
			}
			fmt.Println(":::::::::", v.UserId)
			if tempdata, _ := imdb.GetNft1155FromCommunityRoleDb(v.UserId); err == nil {
				for _, value := range tempdata {
					var tempdataNftValue sdkws.NftTag1155Info
					utils.CopyStructFields(&tempdataNftValue, value)
					pb.NftValue = append(pb.NftValue, &tempdataNftValue)
				}
			}
			resp.UserThirdInfoList = append(resp.UserThirdInfoList, pb)
		}

		if len(req.UserList) > 1 {
			var otherUserSlice []string
			otherUserSlice = append(req.UserList[:index], req.UserList[index+1:]...)
			if len(otherUserSlice) >= 1 {
				result, err = imdb.GetThirdUserInfoWithShowFlag(otherUserSlice, req.ChainID)
				if err != nil {
					resp.CommonResp.ErrCode = constant.ErrDB.ErrCode
					resp.CommonResp.ErrMsg = err.Error()
					return resp, nil
		...
		result, err := imdb.GetThirdUserInfoWithShowFlag(req.UserList, req.ChainID)
		if err != nil {
			resp.CommonResp.ErrCode = constant.ErrDB.ErrCode
			resp.CommonResp.ErrMsg = err.Error()
			return resp, nil
	...
	fmt.Println(">>>>>>>>>>>>>>>>>>>", utils.StructToJsonString(resp))
	return resp, nil
}

func (s *userServer) BindUserEnsDomain(ctx context.Context, req *pbUser.BindUserEnsDomainReq) (*pbUser.BindUserThirdInfoResp, error) {
	//TODO implement me
	type TempEnsCheck struct {
		Address string
		EnsName string
	}
	log.NewInfo(req.OperationID, utils.GetSelfFuncName(), "req: ", req.String())
	resp := &pbUser.BindUserThirdInfoResp{CommonResp: &pbUser.CommonResp{}}
	PostCheckData, _ := json.Marshal(&TempEnsCheck{
		Address: req.OpUserID,
		EnsName: req.EnsDomain,
	})
	resultByte, err := utils.HttpPost(config.Config.EnsPostCheck.Url+"/graph/enscheck",
		"", map[string]string{"Content-Type": "application/json", "chainId": req.ChainID}, PostCheckData)
	if err != nil {
		resp.CommonResp.ErrCode = constant.ErrStatus.ErrCode
		resp.CommonResp.ErrMsg = err.Error()
		return resp, nil
	} else {
		var tempdataresult pbUser.CommonResp
		json.Unmarshal(resultByte, &tempdataresult)
		if tempdataresult.ErrCode != 0 {
			resp.CommonResp.ErrCode = tempdataresult.ErrCode
			resp.CommonResp.ErrMsg = tempdataresult.ErrMsg
			return resp, nil
		}
	}
	err = imdb.BindEnsDomain(req.OpUserID, req.EnsDomain, req.ChainID)
	if err != nil {
		resp.CommonResp.ErrCode = constant.ErrDB.ErrCode
		resp.CommonResp.ErrMsg = err.Error()
		return resp, nil
	...
	if err == nil {
		err = imdb.UpdateUserInfo(db.User{
			UserID: req.OpUserID,
			Ex:     fmt.Sprintf(`{"ensDomain":"%s"}`, req.EnsDomain),
		})
	}
	s.updateInfoToMyFriend(req.OperationID, req.OpUserID)
	return resp, nil
}
func (s *userServer) updateInfoToMyFriend(OperationID string, userID string) {

	etcdConn := getcdv3.GetDefaultConn(config.Config.Etcd.EtcdSchema, strings.Join(config.Config.Etcd.EtcdAddr, ","), config.Config.RpcRegisterName.OpenImFriendName,
		OperationID)
	if etcdConn == nil {
	...
	client := pbFriend.NewFriendClient(etcdConn)
	newReq := &pbFriend.GetFriendListReq{
		CommID: &pbFriend.CommID{OperationID: OperationID, FromUserID: userID, OpUserID: userID},
	}

	rpcResp, err := client.GetFriendList(context.Background(), newReq)
	if err != nil {
		log.NewError(OperationID, "GetFriendList failed ", err.Error(), newReq)
		//return &pbUser.UpdateUserInfoResp{CommonResp: &pbUser.CommonResp{ErrCode: 500, ErrMsg: err.Error()}}, nil
	} else {
		for _, v := range rpcResp.FriendInfoList {
			fmt.Println("\n需要提醒xxxxx 更新我的用户信息", OperationID, "UserInfoUpdatedNotification ", userID, v.FriendUser.UserID)
			chat.UserInfoUpdatedNotification(OperationID, userID, v.FriendUser.UserID)
		}
	}

	rocksCache.DelUserInfoFromCache(userID)
	chat.UserInfoUpdatedNotification(OperationID, userID, userID)

}

func (s *userServer) BindUserThirdInfo(ctx context.Context, req *pbUser.BindUserThirdInfoReq) (*pbUser.BindUserThirdInfoResp, error) {
	resp := &pbUser.BindUserThirdInfoResp{CommonResp: &pbUser.CommonResp{}}
	if req.Action&0x01 == 1 {
		log.NewInfo(req.OperationID, utils.GetSelfFuncName(), "req: ", req.String())
		err := imdb.BindDomain(req.OpUserID, req.Domain)
		if err != nil {
			resp.CommonResp.ErrCode = constant.ErrDB.ErrCode
			resp.CommonResp.ErrMsg = err.Error()
			return resp, nil
	...
	if req.Action>>1&0x01 == 1 {
		log.NewInfo(req.OperationID, utils.GetSelfFuncName(), "req: ", req.String())

		err := imdb.BindUserEmail(req.OpUserID, req.EmailAddress)
		if err != nil {
			resp.CommonResp.ErrCode = constant.ErrDB.ErrCode
			resp.CommonResp.ErrMsg = err.Error()
			return resp, nil
...
func (s *userServer) BindUserTelephoneRPC(ctx context.Context, req *pbUser.BindUserTelephoneReq) (*pbUser.BindUserTelephoneResp, error) {
	//TODO implement me
	resp := &pbUser.BindUserTelephoneResp{CommonResp: &pbUser.CommonResp{}}
	updatemap := make(map[string]interface{}, 0)
	if req.Telephone != "" {
		updatemap["phone_number"] = req.Telephone
	}
	if req.Email != "" {
		updatemap["email"] = req.Email
	}
	err := imdb.UpdateUserPhoneNumber(req.OpUserID, req.Telephone, req.IsUpdatePhone)
	if err != nil {
		resp.CommonResp.ErrCode = constant.ErrDB.ErrCode
		resp.CommonResp.ErrMsg = err.Error()
		return resp, nil
	...
	s.updateInfoToMyFriend(req.OperationID, req.OpUserID)
	// 待优化 通过任务线的kafka去判断
	//if err := imdb.FinishTaskId(req.OpUserID, "2", "绑定了手机号码:"+req.Telephone); err == nil {
	//	if yaoqingren, err := imdb.GetRegisterInfo(req.OpUserID); err == nil && yaoqingren.InvitationCode != "" {
	//		imdb.FinishTaskId(yaoqingren.InvitationCode, "10", "邀请了:"+req.OpUserID+"让其注册了电话号码"+req.Telephone)
	//	}
	//}
	return resp, nil
}

func (s *userServer) DeletePlatformInfo(ctx context.Context, req *pbUser.DelThirdPlatformReq) (*pbUser.DelThirdPlatformResp, error) {
	log.NewInfo(req.OperationID, utils.GetSelfFuncName(), "req: ", req.String())
	resp := &pbUser.DelThirdPlatformResp{CommonResp: &pbUser.CommonResp{}}
	err := imdb.DeletePlatformInfo(req.OpUserID, req.ChainID, req.PlatformName)
	if err != nil {
		resp.CommonResp.ErrCode = constant.ErrDB.ErrCode
		resp.CommonResp.ErrMsg = err.Error()
		return resp, nil
...
func (s *userServer) ShowPlatformInfo(ctx context.Context, req *pbUser.ShowThirdPlatformReq) (*pbUser.ShowThirdPlatformResp, error) {
	log.NewInfo(req.OperationID, utils.GetSelfFuncName(), "req: ", req.String())
	resp := &pbUser.ShowThirdPlatformResp{CommonResp: &pbUser.CommonResp{}}
	err := imdb.ShowPlatformInfo(req.OpUserID, req.ChainID, req.PlatformName, req.ShowFlag)
	if err != nil {
		resp.CommonResp.ErrCode = constant.ErrDB.ErrCode
		resp.CommonResp.ErrMsg = err.Error()
		return resp, nil
...
func (s *userServer) MonitorConversation(ctx context.Context, req *pbUser.SetConversationReq) (*pbUser.SetConversationResp, error) {
	log.NewInfo(req.OperationID, utils.GetSelfFuncName(), "req: ", req.String())
	resp := &pbUser.SetConversationResp{}
	if req.NotificationType == 0 {
		req.NotificationType = constant.ConversationOptChangeNotification
	...
	if req.Conversation.ConversationType == constant.SingleChatType &&
		req.NotificationType == constant.ConversationPrivateChatNotification {
		//sync peer user conversation if conversation is singleChatType
		if err := syncPeerUserConversation(req.Conversation, req.OperationID); err != nil {
			log.NewError(req.OperationID, utils.GetSelfFuncName(), "syncPeerUserConversation", err.Error())
			resp.CommonResp = &pbUser.CommonResp{ErrCode: constant.ErrDB.ErrCode, ErrMsg: constant.ErrDB.ErrMsg}
		...
		if isUpdate {
			chat.ConversationCreateOrUpdateInGroupMonitor(req.OperationID,
				req.Conversation.OwnerUserID, req.Conversation.ConversationID,
				req.Conversation.GroupID, req.Conversation.ConversationType)
		} else {
			chat.ConversationCreateOrUpdateInGroupMonitor(req.OperationID,
				req.Conversation.OwnerUserID, req.Conversation.ConversationID,
				req.Conversation.GroupID, req.Conversation.ConversationType)
