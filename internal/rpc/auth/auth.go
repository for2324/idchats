...
func (rpc *rpcAuth) UserRegister(_ context.Context, req *pbAuth.UserRegisterReq) (*pbAuth.UserRegisterResp, error) {
	...
	AutoCreateCommunity(req.OperationID, user.UserID)
	AutoFollowOfficialSpace(req.OperationID, user.UserID)
...
func AutoCreateCommunity(operationID, userID string) {
	...
	if etcdConn == nil {
		log.NewError(req.OperationID, utils.GetSelfFuncName(), "etcdConn is nil", req.String())
		return
	}
	...
	if RpcResp.ErrCode != 0 {
		log.NewError(req.OperationID, "CreateCommunity failed ", RpcResp.ErrMsg, req.String())
		return
	}
	log.NewInfo(req.OperationID, "CreateCommunity ok ", req.String())
}

func AutoFollowOfficialSpace(operationID, formUserId string) error {
	OfficialSpaceId := config.Config.OfficialSpaceId
	if OfficialSpaceId == "" {
		log.NewError(operationID, "AutoFollowOfficial failed ", "officialGroupId is nil")
		return errors.New("officialGroupId is nil")
	}
	req := &pbFriend.FollowAddFriendReq{
		CommID: &pbFriend.CommID{
			OpUserID:    formUserId,
			FromUserID:  formUserId,
			ToUserID:    OfficialSpaceId,
			OperationID: operationID,
		},
		Follow: true,
	}
	...
	log.NewInfo(operationID, "FollowAddFriend success", formUserId, OfficialSpaceId)
	return nil
}
