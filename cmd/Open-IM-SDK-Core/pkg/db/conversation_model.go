...
func (d *DataBase) GetAllConversationList() ([]*model_struct.LocalConversation, error) {
	...
	//d.conn.Where("conversation_type=3 and is_not_in_group =1").Delete(&model_struct.LocalConversation{})
	var conversationList []model_struct.LocalConversation
	err := utils.Wrap(d.conn.Where("latest_msg_send_time > ?", 0).
...
func (d *DataBase) GetTotalUnreadMsgCount() (totalUnreadCount int32, err error) {
	...
	defer d.mRWMutex.Unlock()
	var result []int64
	err = d.conn.Model(&model_struct.LocalConversation{}).Where("recv_msg_opt < ? AND conversation_type = 1", constant.ReceiveNotNotifyMessage).
...
// GetTotalUnreadGroupMsgCount 获取全部群聊未读消息
func (d *DataBase) GetTotalUnreadGroupMsgCount() (totalUnreadCount int32, err error) {
	d.mRWMutex.Lock()
	defer d.mRWMutex.Unlock()
	var result []int64
	err = d.conn.Model(&model_struct.LocalConversation{}).Where("recv_msg_opt < ? AND conversation_type = 2",
		constant.ReceiveNotNotifyMessage).
		Pluck("unread_count", &result).Error
	if err != nil {
		return totalUnreadCount, utils.Wrap(errors.New("GetTotalUnreadGroupMsgCount err"), "GetTotalUnreadMsgCount err")
	}
	for _, v := range result {
		totalUnreadCount += int32(v)
	}
...
func (d *DataBase) UpdateLastUnReadMessage(conversationID string, lastUnreadMessage int64) (err error) {
	d.mRWMutex.Lock()
	defer d.mRWMutex.Unlock()
	c := model_struct.LocalConversation{ConversationID: conversationID}
	err = d.conn.Debug().Model(&c).Update("update_unread_count_time", lastUnreadMessage).Error
	return
}
func (d *DataBase) GetConversationUnreadCountByconversationID(conversationID string) (count int64, err error) {
	d.mRWMutex.Lock()
	defer d.mRWMutex.Unlock()
	err = d.conn.Model(&model_struct.LocalConversation{}).Where("conversation_id=? and  recv_msg_opt < ?",
		conversationID, constant.ReceiveNotNotifyMessage).
		Pluck("unread_count", &count).Error
	return
