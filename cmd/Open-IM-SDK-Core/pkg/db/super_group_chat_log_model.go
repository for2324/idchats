...
func (d *DataBase) SuperGroupGetMessage(msg *sdk_struct.MsgStruct) (*model_struct.LocalChatLog, error) {
	...
	if msg.ChannelID == "" {
		...
		return &c, utils.Wrap(d.conn.Table(utils.GetSuperGroupTableName(msg.GroupID)).Where("client_msg_id = ? and channel_id=?",
			msg.ClientMsgID, msg.ChannelID).Take(&c).Error, "GetMessage failed")
	}
}
...
func (d *DataBase) SuperGroupGetMessageList(sourceID string, sessionType, count int, startTime int64, isReverse bool, channelID string) (result []*model_struct.LocalChatLog, err error) {
	d.mRWMutex.Lock()
	defer d.mRWMutex.Unlock()
	var messageList []model_struct.LocalChatLog
	...
	if channelID != "" {
		condition = " recv_id = ? AND status <=? And session_type = ? And send_time " + timeSymbol + " ? and channel_id=?"
		err = utils.Wrap(d.conn.Table(utils.GetSuperGroupTableName(sourceID)).Where(
			condition, sourceID, constant.MsgStatusSendFailed, sessionType, startTime, channelID).
...
func (d *DataBase) SuperGroupGetMessageListNoTime(sourceID string, sessionType, count int, isReverse bool, channelId string) (result []*model_struct.LocalChatLog, err error) {
	...
	if channelId != "" {
		...
		err = utils.Wrap(d.conn.Table(utils.GetSuperGroupTableName(sourceID)).Where(
			condition, sourceID, constant.MsgStatusSendFailed, sessionType, channelId).
			Order(timeOrder).Offset(0).Limit(count).Find(&messageList).Error, "GetMessageList failed")
	} else {
		err = utils.Wrap(d.conn.Table(utils.GetSuperGroupTableName(sourceID)).Where(
			condition, sourceID, constant.MsgStatusSendFailed, sessionType).
			Order(timeOrder).Offset(0).Limit(count).Find(&messageList).Error, "GetMessageList failed")
	}
...
func (d *DataBase) SuperGroupUpdateGroupMessageHasRead(msgIDList []string, groupID string) error {
	d.mRWMutex.Lock()
	defer d.mRWMutex.Unlock()
	t := d.conn.Table(utils.GetSuperGroupTableName(groupID)).Where(" client_msg_id in ?", msgIDList).
		Update("is_read", constant.HasRead)
	if t.RowsAffected == 0 {
		return utils.Wrap(errors.New("RowsAffected == 0"), "no update")
	}
	return utils.Wrap(t.Error, "UpdateMessageStatusBySourceID failed")
...
func (d *DataBase) SuperGroupUpdateGroupMessageHasReadWithChannel(groupID string, channelID string) error {
	d.mRWMutex.Lock()
	defer d.mRWMutex.Unlock()
	t := d.conn.Table(utils.GetSuperGroupTableName(groupID)).Where("is_read=0").Update("is_read", constant.HasRead)
	if t.RowsAffected == 0 {
		return utils.Wrap(errors.New("RowsAffected == 0"), "no update")
	}
	return utils.Wrap(t.Error, "UpdateMessageStatusBySourceID failed")
