...
func (d *DataBase) GetMessageListController(sourceID string, sessionType, count int, startTime int64, isReverse bool, channelID string) (result []*model_struct.LocalChatLog, err error) {
	switch sessionType {
	case constant.SuperGroupChatType:
		return d.SuperGroupGetMessageList(sourceID, sessionType, count, startTime, isReverse, channelID)
	default:
...
func (d *DataBase) GetMessageListNoTimeController(sourceID string, sessionType, count int, isReverse bool, channelID string) (result []*model_struct.LocalChatLog, err error) {
	switch sessionType {
	case constant.SuperGroupChatType:
		return d.SuperGroupGetMessageListNoTime(sourceID, sessionType, count, isReverse, channelID)
	default:
...
func (d *DataBase) UpdateSingleMessageHasRead(sendID string, msgIDList []string) error {
	d.mRWMutex.Lock()
	defer d.mRWMutex.Unlock()
	t := d.conn.Model(model_struct.LocalChatLog{}).Where("send_id=?  AND session_type=? AND client_msg_id in ?", sendID, constant.SingleChatType, msgIDList).
