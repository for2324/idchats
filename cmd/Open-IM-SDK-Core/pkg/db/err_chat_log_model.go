...
func (d *DataBase) SuperBatchInsertExceptionMsg(MessageList []*model_struct.LocalErrChatLog, groupID string) error {
	...
	for _, value := range MessageList {
		nValue := int64(0)
		if d.conn.Table(utils.GetSuperGroupTableName(groupID)).Where("Seq=?", value.Seq).Count(&nValue).Error == nil {
			if nValue == 0 {
				err := d.conn.Table(utils.GetSuperGroupTableName(groupID)).Create(value).Error
				if err != nil {
					return utils.Wrap(err, "BatchInsertMessageList failed")
				}
			}
		}
	}
	return utils.Wrap(nil, "BatchInsertMessageList failed")
}
