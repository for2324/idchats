...
func (d *DataBase) DeleteConversationUnreadMessageList(conversationID string, sendTime int64) int64 {
	d.mRWMutex.Lock()
	defer d.mRWMutex.Unlock()
	return d.conn.Where("conversation_id = ? and send_time <= ?", conversationID, sendTime).
		Delete(&model_struct.LocalConversationUnreadMessage{}).RowsAffected
}

//func (d *DataBase) GetSignalChatConversationUnreadMessageList(conversationID string, sendTime int64) int64 {
//	d.mRWMutex.Lock()
//	defer d.mRWMutex.Unlock()
...
//}
//
//func (d *DataBase) GetGroupConversationUnreadMessageList(conversationID string, sendTime int64) int64 {
//	d.mRWMutex.Lock()
//	defer d.mRWMutex.Unlock()
//	return d.conn.Where("conversation_id = ? and send_time <= ?", conversationID, sendTime).Delete(&model_struct.LocalConversationUnreadMessage{}).RowsAffected
//}
