...
func (f *FriendCallback) OnFollowFriendApplicationAdded(friendApplication string) {
	//TODO implement me
	SendOneUserMessage(EventData{cleanUpfuncName(runFuncName()), 0, "", friendApplication, "0"}, f.uid)
}

func (f *FriendCallback) OnFollowFriendApplicationDeleted(friendApplication string) {
	//TODO implement me
	SendOneUserMessage(EventData{cleanUpfuncName(runFuncName()), 0, "", friendApplication, "0"}, f.uid)
}
...
func (wsRouter *WsFuncRouter) FollowAddFriend(paramsReq string, operationID string) {
	userWorker := open_im_sdk.GetUserWorker(wsRouter.uId)
	if !wsRouter.checkResourceLoadingAndKeysIn(userWorker, paramsReq, operationID, runFuncName(), nil) {
		return
	}
	userWorker.Friend().FollowAddFriend(&BaseSuccessFailed{runFuncName(), operationID, wsRouter.uId}, paramsReq, operationID)
}
...
func (wsRouter *WsFuncRouter) GetFollowFriendApplicationList(input string, operationID string) {
	userWorker := open_im_sdk.GetUserWorker(wsRouter.uId)
	if !wsRouter.checkResourceLoadingAndKeysIn(userWorker, input, operationID, runFuncName(), nil) {
		return
	}
	log.Info(operationID, utils.GetSelfFuncName(), "GetFollowFriendApplicationList ", input)
	userWorker.Friend().GetFollowFriendApplicationList(&BaseSuccessFailed{
		runFuncName(),
		operationID,
		wsRouter.uId}, operationID, input)
}
