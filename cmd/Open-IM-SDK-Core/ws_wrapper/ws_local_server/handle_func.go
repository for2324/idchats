...
func (ws *WServer) DoLogin(m Req, conn *UserConn) {
	...
	if !ok {
		...
		refR.refNameMutex.RLock()
		defer refR.refNameMutex.RUnlock()
...
func (ws *WServer) msgParse(conn *UserConn, jsonMsg []byte) {
	...
	if m.ReqFuncName == "Ping" {
		SendOneConnMessage(EventData{"Pong", 0, "ok", "", m.OperationID}, conn)
		return
	}
	parms := []reflect.Value{reflect.ValueOf(m.Data), reflect.ValueOf(m.OperationID)}
	urm.refNameMutex.RLock()
	defer urm.refNameMutex.RUnlock()
