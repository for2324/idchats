...
func (wsRouter *WsFuncRouter) SendMessage(input string, operationID string) {
	...
	userWorker.Conversation().SendMessage(&sc, m["message"].(string),
		m["recvID"].(string),
		m["groupID"].(string),
		m["offlinePushInfo"].(string), channelID, operationID)
...
func (c *ConversationCallback) OnTotalGroupUnreadMessageCountChanged(totalUnreadCount int32) {
	var ed EventData
	ed.Event = cleanUpfuncName(runFuncName())
	ed.ErrCode = 0
	ed.Data = int32ToString(totalUnreadCount)
...
func (wsRouter *WsFuncRouter) GetTotalUnreadGroupMsgCount(input string, operationID string) {
	userWorker := open_im_sdk.GetUserWorker(wsRouter.uId)
	if !wsRouter.checkResourceLoadingAndKeysIn(userWorker, input, operationID, runFuncName(), nil) {
		return
	}
	userWorker.Conversation().GetTotalUnreadGroupMsgCount(&BaseSuccessFailed{runFuncName(), operationID, wsRouter.uId}, operationID)
}
...
func (wsRouter *WsFuncRouter) MarkGroupMessageAsRead(input string, operationID string) {
	...
	if err := json.Unmarshal([]byte(input), &m); err != nil {
		log.Info(operationID, utils.GetSelfFuncName(), "unmarshal failed", input, err.Error())
		wsRouter.GlobalSendMessage(EventData{cleanUpfuncName(runFuncName()), StatusBadParameter,
			"unmarshal failed", "", operationID})
		return
	}
	userWorker := open_im_sdk.GetUserWorker(wsRouter.uId)
	...
	userWorker.Conversation().MarkGroupMessageAsRead(&BaseSuccessFailed{runFuncName(),
		operationID, wsRouter.uId}, m["groupID"].(string), m["msgIDList"].(string), operationID)
}
...
func (wsRouter *WsFuncRouter) CreateAnnouncementMessage(input string, operationID string) {
	m := make(map[string]interface{})
	if err := json.Unmarshal([]byte(input), &m); err != nil {
		log.Info(operationID, utils.GetSelfFuncName(), "unmarshal failed", input, err.Error())
		wsRouter.GlobalSendMessage(EventData{cleanUpfuncName(runFuncName()), StatusBadParameter, "unmarshal failed", "", operationID})
	...
	userWorker := open_im_sdk.GetUserWorker(wsRouter.uId)
	if !wsRouter.checkResourceLoadingAndKeysIn(userWorker, input, operationID, runFuncName(), m, "announcement") {
		return
	}
	var text string
	var imageUrl []string
	...
	msg := userWorker.Conversation().CreateAnnouncementMessage(m["announcement"].(string),
		text, imageUrl, videoUrl, url, operationID)
	wsRouter.GlobalSendMessage(EventData{cleanUpfuncName(runFuncName()), 0, "", msg, operationID})
}
