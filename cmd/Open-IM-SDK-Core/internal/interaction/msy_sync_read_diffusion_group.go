...
func (m *ReadDiffusionGroupMsgSync) doMaxSeq(cmd common.Cmd2Value) {
	...
	log.Info(operationID, "doMaxSeq ", utils.StructToJsonString(cmd.Value.(sdk_struct.CmdMaxSeqToMsgSync)))
	...
	log.Info(operationID, "doMaxSeq ", m.SuperGroupIDList, "need check group list:", groupIDList, m.loginUserID)
	m.superGroupMtx.Lock()
	m.SuperGroupIDList = m.SuperGroupIDList[0:0]
...
func (m *ReadDiffusionGroupMsgSync) syncLatestMsg(operationID string) {
	...
	if flag == 1 {
		...
		log.Info(operationID, "do nothing ", m.loginUserID, "用户加入的群数量", len(m.SuperGroupIDList))
		for _, v := range m.SuperGroupIDList {
...
func (m *ReadDiffusionGroupMsgSync) syncLatestMsgForGroup(groupID, operationID string, loginSync int) {
	log.NewDebug(operationID, utils.GetSelfFuncName(), "syncLatestMsgForGroup start",
		m.loginUserID, groupID, loginSync, m.Group2SyncMsgFinished[groupID], m.Group2SeqMaxNeedSync[groupID], m.Group2SeqMaxSynchronized[groupID])
	if !m.Group2SyncMsgFinished[groupID] {
		...
		//消息最大同步的数据是前面的 constant.PullMsgNumForReadDiffusion 条数据。 默认拉取
...
func (m *ReadDiffusionGroupMsgSync) doPushMsg(cmd common.Cmd2Value) {
	...
	log.Debug(operationID, "recv super group push msg, doPushMsg ", m.loginUserID, msg.Seq, msg.ServerMsgID, msg.ClientMsgID, msg.GroupID, msg.SessionType, m.Group2SeqMaxSynchronized[msg.GroupID], m.Group2SeqMaxNeedSync[msg.GroupID], m.loginUserID)
	if msg.Seq == 0 {
	...
	log.Debug(operationID, "syncMsgFromServer ", m.loginUserID, m.Group2SeqMaxSynchronized[msg.GroupID]+1, m.Group2SeqMaxNeedSync[msg.GroupID])
	//获取此群最新消息，内部保证只调用一次
...
func (m *ReadDiffusionGroupMsgSync) syncMsgFroAllGroup(operationID string) {
	...
	for _, v := range m.SuperGroupIDList {
		...
		if seqMaxNeedSync > seqMaxSynchronized {
			...
			log.Info(operationID, "do nothing ", m.loginUserID, seqMaxSynchronized+1, seqMaxNeedSync, v)
		}
	}
	m.superGroupMtx.Unlock()
}
...
func (m *ReadDiffusionGroupMsgSync) syncMsgForOneGroup(operationID string, groupID string) {
	log.NewDebug(operationID, utils.GetSelfFuncName(), "syncMsgForOneGroup start", m.loginUserID, groupID)
	m.superGroupMtx.Lock()
	for _, v := range m.SuperGroupIDList {
		...
		seqMaxSynchronized := m.Group2SeqMaxSynchronized[v]
		if seqMaxNeedSync > seqMaxSynchronized {
			log.Info(operationID, m.loginUserID, groupID, "do syncMsg ", seqMaxSynchronized+1, seqMaxNeedSync)
			m.syncMsgFromServer(seqMaxSynchronized+1, seqMaxNeedSync, v, operationID, 0)
			m.Group2SeqMaxSynchronized[v] = seqMaxNeedSync
		} else {
	...
	m.superGroupMtx.Unlock()
	log.NewDebug(operationID, utils.GetSelfFuncName(), "syncMsgForOneGroup end", m.loginUserID, groupID)
}
...
func (m *ReadDiffusionGroupMsgSync) syncMsgFromServerSplit(needSyncSeqList []uint32, groupID, operationID string, loginSync int) {
	...
	for {
		...
		if pullMsgReq.OperationID == "" {
			pullMsgReq.OperationID = utils.OperationIDGenerator()
		}
