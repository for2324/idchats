...
func (m *SelfMsgSync) doPushBatchMsg(cmd common.Cmd2Value) {
	...
	for {
		...
		log.Debug(operationID, "max Seq in push batch msg, m.seqMaxNeedSync ", maxSeq, m.seqMaxNeedSync, seqMaxSynchronizedBegin)
...
func (m *SelfMsgSync) triggerCmdNewMsgCome(msgList []*server_api_params.MsgData, operationID string) {
	for {
		//fmt.Println("(m *SelfMsgSync) triggerCmdNewMsgCome(msgList []*server_api_params.MsgData, operationID string) {", string(debug.Stack()))
