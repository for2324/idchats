...
func (f *Friend) followAddFriend(callback open_im_sdk_callback.Base, userIDReqMsg sdk.FollowAddFriendParams, operationID string) {
	log.NewInfo(operationID, utils.GetSelfFuncName(), "args: ", userIDReqMsg)
	apiReq := api.FollowAddFriendReq{}
	apiReq.ToUserID = userIDReqMsg.ToUserID
	apiReq.FromUserID = f.loginUserID
	apiReq.Follow = userIDReqMsg.Follow
	apiReq.OperationID = operationID
	realData := api.FollowAddFriendRsp{
		Follow: apiReq.Follow,
	}
	err := f.p.PostReturn(constant.FollowAddFriendRouter, apiReq, &realData.PublicUserInfo)
	common.CheckDBErrCallback(callback, err, operationID)
	realData.Follow = apiReq.Follow
	log.NewInfo(operationID, utils.GetSelfFuncName(), "return: ", realData)
	f.FollowSyncFriendApplication(operationID, &realData)

}

func (f *Friend) getFollowFriendApplicationList(callback open_im_sdk_callback.Base,
	operationID string, param sdk.FollowListFriendParam) ([]*api.FollowUserInfoList, error) {
	log.NewInfo(operationID, utils.GetSelfFuncName(), "args: ")
	apiReq := api.GetFollowListReq{
		ParamsCommFriend: api.ParamsCommFriend{
			OperationID: operationID,
			ToUserID:    param.ToUserID,
			FromUserID:  f.loginUserID,
		},
		IsFollow: param.SelectFans,
	}
	realData := api.GetFollowListRsp{}
	err := f.p.PostReturn(constant.GetFollowingList, apiReq, &realData.PublicUserInfo)
	common.CheckDBErrCallback(callback, err, operationID)
	log.NewInfo(operationID, utils.GetSelfFuncName(), "return: ", realData)
	return realData.PublicUserInfo, nil
...
func (f *Friend) SyncFriendList(operationID string) {
	...
	for _, index := range aInBNot {
		if index >= len(friendsInfoOnServer) {
			log.Error("painc data is :", len(friendsInfoOnServer), index, f.loginUserID)
			continue
		}
	...
	for _, index := range sameA {
		...
		if err != nil {
			...
			if f.friendListener != nil {
				...
				common.TriggerCmdUpdateConversation(common.UpdateConNode{Action: constant.UpdateConFaceUrlAndNickName,
					Args: common.SourceIDAndSessionType{SourceID: callbackData.FriendUserID, SessionType: constant.SingleChatType}}, f.conversationCh)
				common.TriggerCmdUpdateMessage(common.UpdateMessageNode{Action: constant.UpdateMsgFaceUrlAndNickName,
					Args: common.UpdateMessageInfo{UserID: callbackData.FriendUserID, FaceURL: callbackData.FaceURL, Nickname: callbackData.Nickname}}, f.conversationCh)
...
func (f *Friend) getFriendList(callback open_im_sdk_callback.Base, operationID string) sdk.GetFriendListCallback {
	...
	common.CheckDBErrCallback(callback, err, operationID)
	f.GetFollowEachOtherFriendListCache(callback, operationID) //加载缓存
	return common.MergeFriendBlackResult(localFriendList, localBlackList)
}

func (f *Friend) GetFollowEachOtherFriendListCache(callback open_im_sdk_callback.Base, operationID string) ([]string, error) {
	log.NewInfo(operationID, utils.GetSelfFuncName(), "args: ")
	apiReq := api.GetFollowEachOtherFriendListReq{OperationID: operationID}
	realData := api.GetFollowEachOtherFriendListResp{}
	err := f.p.PostReturn(constant.PostFollowEachOtherFriendCache, apiReq, &realData.Data)
	if err != nil {
		return nil, utils.Wrap(err, apiReq.OperationID)
	}
	log.NewInfo(operationID, utils.GetSelfFuncName(), "return: ", realData.Data)
...
func (f *Friend) GetIsServerFollowEachOtherFriendList(operationID string, toUserID string) (bool, error) {
	log.NewInfo(operationID, utils.GetSelfFuncName(), "args: ")
	apiReq := api.IsFriendReq{
		ParamsCommFriend: api.ParamsCommFriend{
			OperationID: operationID,
			FromUserID:  f.loginUserID,
			ToUserID:    toUserID,
	...
	realData := api.IsFriendResp{
		CommResp: api.CommResp{},
		Response: api.Response{},
	}
	err := f.p.PostReturn(constant.PostIsFollowEachOtherFriend, apiReq, &realData.Response)
	if err != nil {
		return false, utils.Wrap(err, apiReq.OperationID)
	}
	log.NewInfo(operationID, utils.GetSelfFuncName(), "return: ", realData.Response.Friend)
	return realData.Response.Friend, nil
}
...
// recv
func (f *Friend) FollowSyncFriendApplication(operationID string, apivalue *api.FollowAddFriendRsp) {
	if apivalue.Follow {
		f.friendListener.OnFollowFriendApplicationAdded(utils.StructToJsonString(apivalue))
	} else {
		f.friendListener.OnFollowFriendApplicationDeleted(utils.StructToJsonString(apivalue))
	}
}
...
func (f *Friend) DoNotification(msg *api.MsgData, conversationCh chan common.Cmd2Value) {
	...
	go func() {
		...
		case constant.FriendFollowApplicationNotification:
			f.friendfollowApplicationNotification(msg, operationID)
		case constant.FriendFollowDeleteApplicationNotification:
			f.friendfollowdeleteApplicationNotification(msg, operationID)
...
func (f *Friend) friendInfoChangedNotification(msg *api.MsgData, conversationCh chan common.Cmd2Value, operationID string) {
	...
	//	fmt.Println("\ndetail.UserID >>>>>> f.loginUserID  >>>>>>>", detail.UserID, f.loginUserID)
	//	f.SyncFriendList(operationID)
	...
	//				Args: common.UpdateMessageInfo{UserID: detail.UserID, FaceURL: friendInfo.FaceURL, Nickname: friendInfo.Nickname}}, conversationCh)
	//
	//		}
	//	}()
	//
...
func (f *Friend) friendfollowApplicationNotification(msg *api.MsgData, operationID string) {
	log.NewInfo("vvvvvvvvvvv", utils.StructToJsonStringDefault(msg))
	detail := api.FollowFriendTips{}
	if err := comm.UnmarshalTips(msg, &detail); err != nil {
		log.Error(operationID, "comm.UnmarshalTips failed ", err.Error(), msg)
		return
	}

	if f.loginUserID != msg.SendID {
		f.friendListener.OnFollowFriendApplicationAdded(utils.StructToJsonString(detail))
	}

}
func (f *Friend) friendfollowdeleteApplicationNotification(msg *api.MsgData, operationID string) {
	log.NewInfo(operationID, utils.GetSelfFuncName(), "args: ", msg.ClientMsgID, msg.ServerMsgID)
	detail := api.FollowFriendTips{}
	if err := comm.UnmarshalTips(msg, &detail); err != nil {
		log.Error(operationID, "comm.UnmarshalTips failed ", err.Error(), msg)
	...
	log.Info("sendid ,recvid ,loginuserid,:", msg.SendID, msg.RecvID, f.loginUserID)
	if f.loginUserID != msg.SendID {
		f.friendListener.OnFollowFriendApplicationDeleted(utils.StructToJsonString(detail))
	}
