...
func (f *Friend) FollowAddFriend(callback open_im_sdk_callback.Base, userIDReqMsg string, operationID string) {
	if callback == nil {
		log.Error(operationID, "callback is nil")
		return
	}
	...
	go func() {
		...
		var unmarshalAddFriendParams sdk.FollowAddFriendParams
		common.JsonUnmarshalAndArgsValidate(userIDReqMsg, &unmarshalAddFriendParams, callback, operationID)
		f.followAddFriend(callback, unmarshalAddFriendParams, operationID)
		callback.OnSuccess(utils.StructToJsonString(sdk.FollowAddFriendCallback))
		log.NewInfo(operationID, fName, " callback: ", utils.StructToJsonString(sdk.FollowAddFriendCallback))
	}()
}
...
func (f *Friend) GetFollowFriendApplicationList(callback open_im_sdk_callback.Base, operationID string, inputstring string) {
	if callback == nil {
		log.Error(operationID, "callback is nil")
		return
	}
	...
	go func() {
		log.NewInfo(operationID, fName, "args: ")
		var unmarshalParams sdk.FollowListFriendParam
		common.JsonUnmarshalAndArgsValidate(inputstring, &unmarshalParams, callback, operationID)
		result, _ := f.getFollowFriendApplicationList(callback,
			operationID, unmarshalParams)
		callback.OnSuccess(utils.StructToJsonStringDefault(result))
		log.NewInfo(operationID, fName, " callback: ", utils.StructToJsonStringDefault(result))
	}()
}
