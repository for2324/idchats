...
func (c *Conversation) setOneConversationUnreadMerLin(callback open_im_sdk_callback.Base,
	conversationID string, unreadCount int, operationID string, LatestMsgSendTime int64) {
	localConversation, err := c.db.GetConversation(conversationID)
	common.CheckDBErrCallback(callback, err, operationID)
	if localConversation.UnreadCount == 0 {
		return
	}
	err = c.db.DecrConversationUnreadCount(localConversation.ConversationID, int64(unreadCount))
	if err != nil {
		log.Error("internal", "incrConversationUnreadCount database err:", err.Error())
		return
	}
	err = c.db.UpdateLastUnReadMessage(localConversation.ConversationID, LatestMsgSendTime)
	if err != nil {
		log.Error("internal", "incrConversationUnreadCount database err:", err.Error())
		return
	}
	deleteRows := c.db.DeleteConversationUnreadMessageList(localConversation.ConversationID, LatestMsgSendTime)
	if deleteRows == 0 {
	...
	fmt.Println("delete row number", deleteRows)
}
func (c *Conversation) UpdateSuperGroupIsReadAllMessage(conversiontid string) {
	lc, err := c.db.GetConversation(conversiontid)
	if err == nil {
		c.db.SuperGroupUpdateGroupMessageHasReadWithChannel(lc.GroupID, "")
	}
...
func (c *Conversation) deleteConversation(callback open_im_sdk_callback.Base, conversationID, operationID string) {
	...
	//common.CheckDBErrCallback(callback, err, operationID)
	//var sourceID string
	//isSingleChatType := false
	//isGroupChatType := false
	//switch lc.ConversationType {
	//case constant.SingleChatType, constant.NotificationChatType:
	//	sourceID = lc.UserID
	//	isSingleChatType = true
	//case constant.GroupChatType, constant.SuperGroupChatType:
	//	sourceID = lc.GroupID
...
func (c *Conversation) SyncConversations(operationID string, timeout time.Duration) {
	log.Info(operationID, "SyncConversations start")
...
func (c *Conversation) getHistoryMessageList(callback open_im_sdk_callback.Base,
	req sdk.GetHistoryMessageListParams, operationID string, isReverse bool) sdk.GetHistoryMessageListCallback {
	t := time.Now()
	var sourceID string
	...
	if notStartTime {
		list, err = c.db.GetMessageListNoTimeController(sourceID, sessionType, req.Count, isReverse, channelID)
	} else {
		list, err = c.db.GetMessageListController(sourceID, sessionType, req.Count, startTime, isReverse, channelID)
	}
	fmt.Println(">>>>>>>>>>>当前消息剩余多少条", len(list))
...
func (c *Conversation) getAdvancedHistoryMessageList(callback open_im_sdk_callback.Base,
	req sdk.GetAdvancedHistoryMessageListParams, operationID string, isReverse bool) sdk.GetAdvancedHistoryMessageListCallback {
	t := time.Now()
	...
	if len(list) < req.Count && sessionType == constant.SuperGroupChatType {
		var minSeq uint32
		var maxSeq uint32
		resp, err := c.SendReqWaitResp(
			&server_api_params.GetMaxAndMinSeqReq{UserID: c.loginUserID, GroupIDList: []string{sourceID}},
			constant.WSGetNewestSeq,
			1, 2,
			c.loginUserID, operationID)
		if err != nil {
		...
		if seq != 0 {
			if seq <= minSeq {
				...
				if len(seqList) > 0 {
					c.pullMessageAndReGetHistoryMessages(sourceID, seqList, notStartTime, isReverse, req.Count, sessionType,
						startTime, &list, &messageListCallback, operationID, channelID)
				}
			}
		} else {
		...
		if maxSeq != 0 && minSeq != 0 {
			...
			if lostSeqListLength > 0 {
				...
				c.pullMessageAndReGetHistoryMessages(sourceID, pullSeqList, notStartTime, isReverse,
					req.Count, sessionType, startTime, &list, &messageListCallback, operationID, channelID)
			} else {
				if req.LastMinSeq != 0 {
					...
					if thisMaxSeq != 0 {
						if thisMaxSeq+1 != req.LastMinSeq {
							...
							if len(successiveSeqList) > 0 {
								c.pullMessageAndReGetHistoryMessages(sourceID, successiveSeqList, notStartTime,
									isReverse, req.Count, sessionType, startTime, &list, &messageListCallback, operationID, channelID)
							}
						}
...
// 2、块中连续性检测
// 3、块之间连续性检测
func (c *Conversation) pullMessageAndReGetHistoryMessages(sourceID string,
	seqList []uint32, notStartTime, isReverse bool, count,
	sessionType int, startTime int64, list *[]*model_struct.LocalChatLog,
	messageListCallback *sdk.GetAdvancedHistoryMessageListCallback, operationID string, channelID string) {
	...
	if err != nil {
		...
		if err != nil {
			...
			if notStartTime {
				*list, err = c.db.GetMessageListNoTimeController(sourceID, sessionType, count, isReverse, channelID)
			} else {
				*list, err = c.db.GetMessageListController(sourceID, sessionType, count, startTime, isReverse, channelID)
			}
		}
...
func (c *Conversation) markC2CMessageAsRead(callback open_im_sdk_callback.Base, msgIDList sdk.MarkC2CMessageAsReadParams, userID, operationID string) {
	...
	for _, v := range messages {
		if v.IsRead == false && v.ContentType < constant.NotificationBegin && v.SendID != c.loginUserID {
			...
			if lastReadTime < v.SendTime {
				lastReadTime = v.SendTime
			}
		}
	}
	...
	//if isfriend, _ := c.friend.GetIsServerFollowEachOtherFriendList(operationID, userID); !isfriend {
	//	err2 := c.db.UpdateSingleMessageHasReadDelete(userID, newMessageIDList)
	//	if err2 != nil {
	...
	fmt.Println("newmessage:lent", len(messages), "localMessage.SendTime:", localMessage.SendTime, localMessage.Content, lastReadTime)
	c.setOneConversationUnreadMerLin(callback, conversationID, len(newMessages), operationID, lastReadTime)
	...
	_ = common.TriggerCmdUpdateConversation(common.UpdateConNode{ConID: conversationID, Action: constant.TotalUnreadMessageChanged}, c.GetCh())
...
func (c *Conversation) markGroupMessageAsRead(callback open_im_sdk_callback.Base, msgIDList sdk.MarkGroupMessageAsReadParams, groupID, operationID string) {
	...
	if len(msgIDList) == 0 {
		fmt.Println("markGroupMessageAsRead 1")
		c.setOneConversationUnread(callback, conversationID, 0, operationID)
		c.UpdateSuperGroupIsReadAllMessage(conversationID)
		...
		//_ = common.TriggerCmdUpdateConversation(common.UpdateConNode{ConID: conversationID, Action: constant.UnreadGroupMsgSetZero}, c.GetCh())
		_ = common.TriggerCmdUpdateConversation(common.UpdateConNode{ConID: conversationID, Action: constant.ConChange, Args: []string{conversationID}}, c.GetCh())
		return
	}
	var localMessage model_struct.LocalChatLog
	...
	if len(allUserMessage) == 0 {
		// common.CheckAnyErrCallback(callback, 201, errors.New("message has been marked read or sender is yourself or notification message not support"), operationID)
		common.CheckAnyErrCallback(callback, 0, errors.New("message has been marked read or sender is yourself or notification message not support"), operationID)
	}
	...
	for userID, list := range allUserMessage {
		...
		_ = common.TriggerCmdUpdateConversation(common.UpdateConNode{ConID: conversationID, Action: constant.DecrUnRead,
			Args: int(1)}, c.GetCh())
		_ = common.TriggerCmdUpdateConversation(common.UpdateConNode{ConID: conversationID, Action: constant.ConChange,
			Args: []string{conversationID}}, c.GetCh())
	}
}
...
func (c *Conversation) deleteMessageFromLocalStorage(callback open_im_sdk_callback.Base, s *sdk_struct.MsgStruct, operationID string) {
	...
	if s.ClientMsgID == latestMsg.ClientMsgID { //If the deleted message is the latest message of the conversation, update the latest message of the conversation
		list, err := c.db.GetMessageListNoTimeController(sourceID, int(s.SessionType), 1, false, channelID)
		common.CheckDBErrCallback(callback, err, operationID)
