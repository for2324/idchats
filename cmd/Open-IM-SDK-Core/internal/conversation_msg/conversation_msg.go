...
func (c *Conversation) doMsgNew(c2v common.Cmd2Value) {
	fmt.Println("doMsgNew>>", utils.StructToJsonString(c2v))
	fmt.Println("dealwiht:>>", c.loginUserID)
	...
	for _, v := range allMsg {
		log.Info(operationID, "do Msg come here, msg detail ",
			v.RecvID, v.SendID, v.ClientMsgID, v.ServerMsgID, v.Seq, c.loginUserID)
		...
		fmt.Println("msg ClientMsgID:", msg.ClientMsgID)
		if msg.ClientMsgID == "" {
		...
		case v.ContentType == constant.ConversationUnreadNotification:
			...
			fmt.Println(operationID, "ConversationUnreadNotification come here", unreadArgs.String())
			...
			continue
		case v.ContentType == constant.ConversationMonitorForGroupNotification:
			var monitorConversation server_api_params.ConversationMonitorForGroupTips
			_ = proto.Unmarshal(tips.Detail, &monitorConversation)
			c.createNewConversationMonitorForGroup(monitorConversation.ConversationID, monitorConversation.SourceID, monitorConversation.SessionType)
			continue
		}
		fmt.Println("v.SessionType:", v.SessionType)
		...
		case constant.SingleChatType:
			if v.ContentType > constant.FriendNotificationBegin && v.ContentType < constant.FriendNotificationEnd {
				...
				fmt.Println(operationID, "DoFriendMsg  DoUserMsg SingleChatType", utils.StructToJsonString(v))
				if v.SendID == v.RecvID {
		...
		case constant.GroupChatType, constant.SuperGroupChatType:
			if v.ContentType > constant.GroupNotificationBegin && v.ContentType < constant.GroupNotificationEnd {
				log.Info(operationID, c.loginUserID, "DoGroupMsg GroupChatType", v.ContentType)
				c.group.DoNotification(v, c.GetCh())
				log.Info(operationID, c.loginUserID, "DoGroupMsg GroupChatType", v.ContentType)
			} else if v.ContentType > constant.SignalingNotificationBegin && v.ContentType < constant.SignalingNotificationEnd {
				log.Info(operationID, "signaling DoNotification ", v)
		...
		if v.SendID == c.loginUserID { //seq
			fmt.Println(" v.SendID== c.loginUserID")
			// Messages sent by myself
			//if  sent through  this terminal
			...
			if err == nil {
				...
				fmt.Println("isConversationUpdate>>>>>>:", isConversationUpdate)
				fmt.Println("isSenderConversationUpdate>>>>>>:", isSenderConversationUpdate)
				if isConversationUpdate {
					if isSenderConversationUpdate {
						fmt.Println(operationID, "updateConversation msg", v, lc)
			...
			fmt.Println("==============Sent by others===============isConversationUpdate:", isConversationUpdate) //Sent by others
			if _, err := c.db.GetMessageController(msg); err != nil {                                             //Deduplication operation
				fmt.Println("==============Sent by others===============eeeeeeeee")
				lc := model_struct.LocalConversation{
					ConversationType:  v.SessionType,
					LatestMsg:         utils.StructToJsonString(msg),
					LatestMsgSendTime: msg.SendTime,
				...
				if isUnreadCount {
					...
					if msg.SendTime > cacheConversation.UpdateUnreadCountTime {
						...
						lc.UnreadCount = 1
						if v.SessionType == constant.GroupChatType || v.SessionType == constant.SuperGroupChatType {
							isGroupUnread = true
						}
						tempUnreadMessages := model_struct.LocalConversationUnreadMessage{ConversationID: lc.ConversationID,
							ConversationType: v.SessionType,
				...
				exceptionMsg = append(exceptionMsg, c.msgStructToLocalErrChatLog(msg))
				fmt.Println(operationID, "Deduplication operation ", *c.msgStructToLocalErrChatLog(msg))
			}
		}
	}
	...
	listToMap(list, m)
	log.Debug(operationID, "conversation listToMap: ", c.loginUserID, len(list), len(conversationSet))

	fmt.Println("LocalConversation  -----mm")
	for key, _ := range m {
		fmt.Println(key)
	}
	fmt.Println("LocalConversation  -----mm")

	fmt.Println("conversationSet  -----mm")
	for key, _ := range conversationSet {
		fmt.Println(key)
	}
	fmt.Println("conversationSet  -----mm")
	fmt.Println("newConversationSet  -----mm")
	for key, _ := range newConversationSet {
		fmt.Println(key)
	}
	fmt.Println("newConversationSet  -----mm", len(conversationChangedSet))

	c.diff(m, conversationSet, conversationChangedSet, newConversationSet)
	fmt.Println("======================分割线=====================")
	fmt.Println("LocalConversation  -----mm", len(conversationChangedSet))
	for key, _ := range m {
		fmt.Println(key)
	}
	fmt.Println("LocalConversation  -----mm")
	...
	fmt.Println(operationID, "trigger map is :", "newConversations", newConversationSet, "changedConversations", conversationChangedSet)
	...
	log.Info(operationID, "BatchUpdateMessageList, cost time : ", len(insertMsg))
	...
	if isGroupUnread {
		c.doUpdateConversation(common.Cmd2Value{Value: common.UpdateConNode{"", constant.TotalGroupUnreadMessageChanged, ""}})
	} else {
...
func (c *Conversation) doSuperGroupMsgNew(c2v common.Cmd2Value) {
	log.Debug("xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx")
	operationID := c2v.Value.(sdk_struct.CmdNewMsgComeToConversation).OperationID
	allMsg := c2v.Value.(sdk_struct.CmdNewMsgComeToConversation).MsgList
	syncFlag := c2v.Value.(sdk_struct.CmdNewMsgComeToConversation).SyncFlag
	...
	for _, v := range allMsg {
		...
		case v.ContentType == constant.ConversationUnreadNotification:
			...
			_ = proto.Unmarshal(tips.Detail, &unreadArgs)
			for _, v := range unreadArgs.ConversationIDList {
				fmt.Println("ConversationUnreadNotification 3")
				c.doUpdateConversation(common.Cmd2Value{Value: common.UpdateConNode{ConID: v, Action: constant.UnreadCountSetZero}})
				c.db.DeleteConversationUnreadMessageList(v, unreadArgs.UpdateUnreadCountTime)
			}
			c.doUpdateConversation(common.Cmd2Value{Value: common.UpdateConNode{Action: constant.ConChange, Args: unreadArgs.ConversationIDList}})
			...
			if oldMessage, err := c.db.GetMessageController(msg); err != nil { //Deduplication operation
				...
				if isUnreadCount {
					...
					if msg.SendTime > cacheConversation.UpdateUnreadCountTime {
						isTriggerUnReadCount = true
						lc.UnreadCount = 1
						tempUnreadMessages := model_struct.LocalConversationUnreadMessage{
				...
				if isConversationUpdate {
					fmt.Println("conversationSet:", len(conversationSet))
					c.updateConversation(&lc, conversationSet)
					fmt.Println("conversationSet:", len(conversationSet))
					newMessages = append(newMessages, msg)
				} else {
	...
	b3 := utils.GetCurrentTimestampByMill()
	log.Debug(operationID, "BatchUpdateMessageList, cost time : ", b3-b2)
	log.Debug(operationID, "SuperGroupMsg  BatchUpdateMessageList, cost time : ", len(insertMsg))
	//Normal message storage
	err1 := c.db.BatchInsertMessageListController(insertMsg)
	if err1 != nil {
		log.Error(operationID, "insert GetMessage detail err:", err1.Error(), len(insertMsg))
	...
	for _, v := range hList {
		if nc, ok := newConversationSet[v.ConversationID]; ok {
			...
			nc.AttachedInfo = v.AttachedInfo
			nc.Ex = v.Ex
			//phConversationChangedSet[v.ConversationID].LatestMsgSendTime = v.LatestMsgSendTime
			//phConversationChangedSet[v.ConversationID].LatestMsg = v.LatestMsg
			//phConversationChangedSet[v.ConversationID].UnreadCount = v.UnreadCount
			//phConversationChangedSet[v.ConversationID].DraftText = v.DraftText
			//phConversationChangedSet[v.ConversationID].DraftTextTime = v.DraftTextTime
		}
	}
	b5 := utils.GetCurrentTimestampByMill()
	log.Debug(operationID, "GetHiddenConversationList, cost time : ", b5-b4)
	...
	fmt.Println("隐藏>>>>>>>>>>>>>>>>>>>>phConversationChangedSet:", len(phConversationChangedSet))
	//Changed conversation storage
	err3 := c.db.BatchUpdateConversationList(append(mapConversationToList(conversationChangedSet), mapConversationToList(phConversationChangedSet)...))
	if err3 != nil {
		log.Error(operationID, "insert changed conversation err :", err3.Error())
...
func (c *Conversation) doDeleteConversation(c2v common.Cmd2Value) {
	...
	dtconversionInfo, err := c.db.GetConversation(node.ConversationID)
	if dtconversionInfo != nil {
		if dtconversionInfo.GroupID != "" {
			c.doUpdateConversation(common.Cmd2Value{Value: common.UpdateConNode{"", constant.TotalGroupUnreadMessageChanged, ""}})
		} else {
			c.doUpdateConversation(common.Cmd2Value{Value: common.UpdateConNode{"", constant.TotalUnreadMessageChanged, ""}})
		}
...
func (c *Conversation) doUpdateConversation(c2v common.Cmd2Value) {
		...
		if err == nil {
			...
			if lc.LatestMsgSendTime >= oc.LatestMsgSendTime { //The session update of asynchronous messages is subject to the latest sending time
				...
				if err != nil {
					...
					list = append(list, oc)
					fmt.Println("=========>>>", c.loginUserID, len(list))
			...
			if err4 != nil {
				...
				list = append(list, &lc)
				log.Info("internal", "constant.AddConOrUpLatMsg success :", c.loginUserID, string(debug.Stack()))
	...
	case constant.DecrUnRead:
		subCount := int64(node.Args.(int))
		unreadCount, err := c.db.GetConversationUnreadCountByconversationID(node.ConID)
		if subCount > unreadCount {
			subCount = unreadCount
		}
		if subCount == 0 {
			return
		}
		err = c.db.DecrConversationUnreadCount(node.ConID, subCount)
		if err != nil {
			log.Error("internal", "incrConversationUnreadCount database err:", err.Error())
			return
		}
	...
	case constant.TotalGroupUnreadMessageChanged:
		totalUnreadCount, err := c.db.GetTotalUnreadGroupMsgCount()
		if err != nil {
			log.Error("internal", "OnTotalGroupUnreadMessageCountChanged database err:", err.Error())
		} else {
			c.ConversationListener.OnTotalGroupUnreadMessageCountChanged(totalUnreadCount)
		}
	...
	case constant.UpdateLatestMessageChange:
		fmt.Println("======================================================122222222,UpdateLatestMessageChange")
		...
		if err != nil {
			...
			if err != nil {
				...
				fmt.Println(latestMsg.SendTime, utils.StructToJsonStringDefault(newLatestMessage))
	...
	case constant.ConChange:
		...
		if err != nil {
			...
			log.Info("internal", "getMultipleConversationModel success :", len(newCList))
			fmt.Println("=========>>>", c.loginUserID, len(newCList))
	...
	case constant.NewCon:
		...
		if err != nil {
			...
			if cLists != nil {
				log.Info("internal", "getMultipleConversationModel success :", c.loginUserID, len(cLists), string(debug.Stack()))
				c.ConversationListener.OnNewConversation(utils.StructToJsonString(cLists))
			}
		}
	case constant.ConChangeDirect:
		cidList := node.Args.(string)
		fmt.Println("=========>>>", c.loginUserID, len(cidList))
		c.ConversationListener.OnConversationChanged(cidList)
	case constant.NewConDirect:
		cidList := node.Args.(string)
		log.Info("internal", "constant.NewConDirect success :", c.loginUserID, string(debug.Stack()))
	...
	case constant.ConversationLatestMsgHasRead:
		...
		for conversationID, msgIDList := range hasReadMsgList {
			...
			fmt.Println(">>>>>>>>>>>>>>>>>>>>>>>>", "ConversationLatestMsgHasRead:ConversationLatestMsgHasRead")
...
func (c *Conversation) Work(c2v common.Cmd2Value) {
	...
	case constant.CmdNewMsgCome:
		fmt.Println("internal", "doMsgNew start..", c2v.Cmd)
		c.doMsgNew(c2v)
		fmt.Println("internal", "doMsgNew end..", c2v.Cmd)
...
func (c *Conversation) msgHandleByContentType(msg *sdk_struct.MsgStruct) (err error) {
	...
	if msg.ContentType >= constant.NotificationBegin && msg.ContentType <= constant.NotificationEnd {
		...
		//msg.NotificationElem = new(sdk_struct.NotificationElem)
		...
		case constant.Announcement:
			msg.AnnouncementElem = new(sdk_struct.AnnouncementElem)
			err = utils.JsonStringToStruct(msg.Content, &msg.AnnouncementElem)
		}
	}
