...
func (c *Conversation) SetConversationDraft(callback open_im_sdk_callback.Base, conversationID, draftText string, operationID string) {
	...
	go func() {
		fmt.Println("SetConversationDraft  SetConversationDraft   SetConversationDraft  ")
...
func (c *Conversation) ResetConversationGroupAtType(callback open_im_sdk_callback.Base, conversationID, operationID string) {
	...
	go func() {
		...
		c.setOneConversationGroupAtType(callback,
...
func (c *Conversation) GetTotalUnreadGroupMsgCount(callback open_im_sdk_callback.Base, operationID string) {
	if callback == nil {
		return
	}
	go func() {
		log.NewInfo(operationID, "GetTotalUnreadGroupMsgCount args: ")
		count, err := c.db.GetTotalUnreadGroupMsgCount()
		common.CheckDBErrCallback(callback, err, operationID)
		callback.OnSuccess(utils.Int32ToString(count))
		log.NewInfo(operationID, "GetTotalUnreadGroupMsgCount callback: ", utils.Int32ToString(count))
	}()
}
...
func (c *Conversation) CreateAnnouncementMessage(ipfsAddress, text string, imageUrl, videoUrl, Url []string, operationID string) string {
	s := sdk_struct.MsgStruct{}
	s.InitStruct()
	c.initBasicInfo(&s, constant.UserMsgType, constant.Announcement, operationID)

	s.AnnouncementElem.Announcement = ipfsAddress
	s.AnnouncementElem.Text = text
	s.AnnouncementElem.PicturePath = append(s.AnnouncementElem.PicturePath, imageUrl...)
	s.AnnouncementElem.VideoPath = append(s.AnnouncementElem.VideoPath, videoUrl...)
	s.AnnouncementElem.UrlPath = append(s.AnnouncementElem.PicturePath, Url...)
	s.Content = utils.StructToJsonString(s.AnnouncementElem)
	return utils.StructToJsonString(s)
}
...
func (c *Conversation) SendMessage(callback open_im_sdk_callback.SendMsgCallBack, message, recvID, groupID string,
	offlinePushInfo string, channelId string, operationID string) {
	if callback == nil {
		return
	}
	go func() {
		...
		if recvID == "" {
			...
			case constant.SuperGroup, constant.WorkingGroup:
				...
				if lc.ChannelID == "1" && g.CreatorUserID != c.loginUserID {
					common.CheckAnyErrCallback(callback, 301, errors.New("禁止在公告频道发言"), operationID)
				}
				if g.GroupType == constant.WorkingGroup && channelId == "" {
					common.CheckAnyErrCallback(callback, 302, errors.New("禁止在不选择频道发言"), operationID)
				} else if g.GroupType == constant.WorkingGroup && utils.StringToInt64(channelId) > 10 {
					apiReq := server_api_params.CommunityChannelStatusReq{
						OperationID: operationID,
						GroupID:     g.GroupID,
						ChannelID:   channelId,
					}
					apiResp := server_api_params.CommunityChannelStatusResp{}
					c.p.PostReturn(constant.CommunityChannelStatus, apiReq, &apiResp.Data)
					if apiResp.Data == constant.GroupStatusDismissed {
						common.CheckAnyErrCallback(callback, 302, errors.New("禁止在频道发言"), operationID)
					}
				}
			}
...
func (c *Conversation) CreateSoundMessageByURL(soundBaseInfo, operationID string) string {
	...
	s.SoundElem = new(sdk_struct.SoundElem)
	utils.CopyStructFields(s.SoundElem, &soundElem)
	c.initBasicInfo(&s, constant.UserMsgType, constant.Voice, operationID)
	s.Content = utils.StructToJsonString(s.SoundElem)
	return utils.StructToJsonString(s)
}
...
func (c *Conversation) CreateVideoMessageByURL(videoBaseInfo, operationID string) string {
	...
	s.VideoElem = new(sdk_struct.VideoElem)
	utils.CopyStructFields(s.VideoElem, &videoElem)
	c.initBasicInfo(&s, constant.UserMsgType, constant.Video, operationID)
	s.Content = utils.StructToJsonString(s.VideoElem)
	return utils.StructToJsonString(s)
}
...
func (c *Conversation) CreateFileMessageByURL(fileBaseInfo, operationID string) string {
	...
	s.FileElem = new(sdk_struct.FileElem)
	utils.CopyStructFields(s.FileElem, &fileElem)
	c.initBasicInfo(&s, constant.UserMsgType, constant.File, operationID)
	s.Content = utils.StructToJsonString(s.FileElem)
	return utils.StructToJsonString(s)
}
...
func (c *Conversation) GetHistoryMessageList(callback open_im_sdk_callback.Base, getMessageOptions, operationID string) {
	...
	go func() {
		...
		var respCallback sdk_params_callback.GetHistoryMessageListCallback
		...
		// 	// 群聊
		// 	err = c.p.PostReturn(constant.GetGroupAllHistoryMessageList, &unmarshalParams, &respCallback)
		// } else {
		// 	// 私聊
		// 	err = c.p.PostReturn(constant.GetSinglerChatAllHistoryMessageList, &unmarshalParams, &respCallback)

		// }
		// if err != nil {
		// 	log.Error(operationID, "GetHistoryMessageList err: ", err.Error())
		// 	callback.OnError(constant.ErrServerReturn.ErrCode, err.Error())
		// 	return
		// }
		// var resolveResp []*sdk_struct.MsgStruct
		// for _, v := range respCallback {
		// 	err := c.msgHandleByContentType(v)
		// 	if err != nil {
		// 		log.Error(operationID, "Parsing data error:", err.Error())
		// 		continue
		// 	}
		// 	resolveResp = append(resolveResp, v)
		// }
		// callback.OnSuccess(utils.StructToJsonStringDefault(resolveResp))
		log.NewInfo(operationID, "cost time", time.Since(t), "GetHistoryMessageList callback: ", utils.StructToJsonStringDefault(respCallback))
	}()
}
...
func (c *Conversation) MarkC2CMessageAsRead(callback open_im_sdk_callback.Base, userID string, msgIDList, operationID string) {
	...
	go func() {
		...
		if len(unmarshalParams) == 0 {
			...
			fmt.Println("MarkC2CMessageAsRead 4")
...
func (c *Conversation) MarkMessageAsReadByConID(callback open_im_sdk_callback.Base, conversationID, msgIDList, operationID string) {
	...
	go func() {
		...
		common.JsonUnmarshalCallback(msgIDList, &unmarshalParams, callback, operationID)
		if len(unmarshalParams) == 0 {
			fmt.Println("MarkMessageAsReadByConID 5")
			c.setOneConversationUnread(callback, conversationID, 0, operationID)
			_ = common.TriggerCmdUpdateConversation(common.UpdateConNode{ConID: conversationID, Action: constant.UnreadCountSetZero}, c.GetCh())
			_ = common.TriggerCmdUpdateConversation(common.UpdateConNode{ConID: conversationID, Action: constant.ConChange, Args: []string{conversationID}}, c.GetCh())
...
func (c *Conversation) MarkGroupMessageHasRead(callback open_im_sdk_callback.Base, groupID string, operationID string) {
	...
	go func() {
		...
		conversationID := utils.GetConversationIDBySessionType(groupID, constant.GroupChatType)
		dbGroupInfo, _ := c.db.GetGroupInfoByGroupID(groupID)
		if dbGroupInfo != nil {
			if dbGroupInfo.GroupType == constant.WorkingGroup {
				conversationID = utils.GetConversationIDBySessionType(groupID, constant.SuperGroupChatType)
			}
		}
		c.setOneConversationUnread(callback, conversationID, 0, operationID)
...
func (c *Conversation) getConversationTypeByGroupTypeAndGroupId(groupIDType int, groupID string) (conversationID string, conversationType int32, err error) {
	switch groupIDType {
	case constant.NormalGroup:
		return utils.GetConversationIDBySessionType(groupID, constant.GroupChatType), constant.GroupChatType, nil
	case constant.SuperGroup, constant.WorkingGroup:
...
func (c *Conversation) createNewConversationMonitorForGroup(conversationID string, sourceID string, sessionType int32) {

	lc, err := c.db.GetConversation(conversationID)
	if err == nil {
		g, err2 := c.full.GetGroupInfoFromLocal2Svr(sourceID, sessionType)
		if err2 == nil {
			_ = c.db.CheckAndInsertMonitorGroup(g)
		}
		fmt.Println("createNewConversationMonitorForGroup>>>>>>>>>>>TriggerCmdUpdateConversation>>>>>>>>>createNewConversationMonitorForGroup")
		_ = common.TriggerCmdUpdateConversation(common.UpdateConNode{ConID: lc.ConversationID,
			Action: constant.AddConOrUpLatMsg, Args: *lc}, c.GetCh())
	} else {
		...
		case constant.SuperGroupChatType:
			...
			if err2 == nil {
				...
				_ = c.db.CheckAndInsertMonitorGroup(g)
				_ = c.db.InsertConversation(&newConversation)
				_ = common.TriggerCmdUpdateConversation(common.UpdateConNode{ConID: lc.ConversationID, Action: constant.AddConOrUpLatMsg, Args: *lc}, c.GetCh())
			} else {
				fmt.Println("createNewConversationMonitorForGroup>>>", err2.Error())
			}
