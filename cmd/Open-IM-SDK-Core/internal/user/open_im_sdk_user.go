...
func (u *User) GetSelfUserInfo(callback open_im_sdk_callback.Base, operationID string) {
	...
	go func() {
		...
		var respResult *sdk_params_callback.GetSelfUserAndGroupInfoCallback
		err := u.p.PostReturn(constant.GetSelfUserInfoRouter, apiReq, &respResult)
		if err != nil {
			log.Error(operationID, "GetSelfUserInfo failed ", err.Error())
			callback.OnError(500, err.Error())
			return
		}
		callback.OnSuccess(utils.StructToJsonString(respResult))
		log.NewInfo(operationID, fName, "callback: ", utils.StructToJsonString(respResult))
	}()
}
