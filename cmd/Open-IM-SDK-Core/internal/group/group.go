...
func (g *Group) DoNotification(msg *api.MsgData, conversationCh chan common.Cmd2Value) {
	...
	go func() {
		...
		case constant.GroupMemberSetToOrdinaryUserNotification:
			g.groupMemberInfoSetNotification(msg, operationID) //ok
		case constant.GroupChannelInfoAdd:
			g.groupChannelChangeNotification(msg, operationID)
		case constant.GroupChannelInfoUpdate:
			g.groupChannelChangeNotification(msg, operationID)
		case constant.GroupChannelInfoDiss:
			g.groupChannelChangeNotification(msg, operationID)
		default:
...
func (g *Group) groupMemberInfoSetNotification(msg *api.MsgData, operationID string) {
	...
	g.syncGroupMemberByGroupID(detail.Group.GroupID, operationID, true)
	if msg.ChannelID != "" {
		_ = g.db.UpdateMsgSenderFaceURLAndSenderNickname(detail.ChangedUser.UserID, detail.ChangedUser.FaceURL, detail.ChangedUser.Nickname, constant.SuperGroupChatType)
	} else {
...
func (g *Group) groupChannelChangeNotification(msg *api.MsgData, operatorID string) {
	detail := api.GroupChannelInfoTips{OpUser: &api.GroupMemberFullInfo{}, OpInfo: "",
		GroupChannelInfoTip: &api.GroupChannelInfo{}}
	if err := comm.UnmarshalTips(msg, &detail); err != nil {
		log.Error(operatorID, "UnmarshalTips failed ", err.Error(), msg)
		return
	}
	switch detail.OpInfo {
	case "add":
		fmt.Println("GroupChannelInfoAdd", utils.StructToJsonString(detail))
		if g.listener != nil {
			g.listener.OnGroupChannelInfoAdded(utils.StructToJsonString(detail))
		}
	case "del":
		fmt.Println("GroupChannelInfoDiss", utils.StructToJsonString(detail))
		if g.listener != nil {
			g.listener.OnGroupChannelInfoDelete(utils.StructToJsonString(detail))
		}
	case "update":
		fmt.Println("GroupChannelInfoUpdate", utils.StructToJsonString(detail))
		if g.listener != nil {
			g.listener.OnGroupChannelInfoUpdate(utils.StructToJsonString(detail))
		}

	}
	log.NewInfo(operatorID, utils.GetSelfFuncName(), "args: ",
		msg.ClientMsgID, msg.ServerMsgID, msg.String(), "detail : ", detail.String())
...
func (g *Group) subscribeGroupChannel(groupID string, callback open_im_sdk_callback.Base, operationID string) {
	g.subGroup(operationID, groupID)
}

func (g *Group) unSubscribeGroupChannel(groupID string, callback open_im_sdk_callback.Base, operationID string) {
...
func (g *Group) getGroupChannelList(callback open_im_sdk_callback.Base, groupID, operationID string) []map[string]interface{} {
	apiReq := api.GetCommunityChannelListReq{}
	apiReq.GroupID = groupID
	apiReq.OperationID = operationID
	realData := api.GetCommunityChannelListResp{}
	g.p.PostFatalCallback(callback, constant.GetCommunityChannelInfoRouter, apiReq,
		&realData.GroupChannelInfoList, apiReq.OperationID)
	return utils.JsonDataList(realData.GroupChannelInfoList)
}
...
type LocalGroupWithThreeMember struct {
	model_struct.LocalGroup
	MemberList []*model_struct.LocalGroupMember `json:"memberList"`
}

func (g *Group) getJoinedGroupListEx(callback open_im_sdk_callback.Base, operationID string) []*LocalGroupWithThreeMember {
	groupList, err := g.db.GetJoinedGroupList()
	log.Info(operationID, utils.GetSelfFuncName(), " args ", groupList)
	common.CheckDBErrCallback(callback, err, operationID)
	superGroupList, _ := g.db.GetJoinedSuperGroupList()
	groupList = append(groupList, superGroupList...)
	resultdata := make([]*LocalGroupWithThreeMember, 0)
	for _, value := range groupList {
		var tempdata = new(LocalGroupWithThreeMember)
		utils.CopyStructFields(tempdata, value)
		tempMemberList, err := g.db.GetGroupMemberListSplit(value.GroupID, 0, 0, 3)
		if err == nil && len(tempMemberList) > 0 {
			tempdata.MemberList = append(tempdata.MemberList, tempMemberList...)
		}
		resultdata = append(resultdata, tempdata)
	}
	return resultdata
}
...
func (g *Group) SyncSelfGroupApplication(operationID string) {
	...
	fmt.Println("currlent len:", len(onLocal), len(onServer))
...
func (g *Group) SyncJoinedGroupList(operationID string) {
	...
	for _, index := range bInANot {
		...
		log.Info(operationID, "OnJoinedGroupDeleted111111111111")
...
func (g *Group) subGroup(operationID string, subGroupId string) {
	log.NewInfo(operationID, utils.GetSelfFuncName(), "args: ")
	groupInfos, err := g.getGroupsInfoFromSvr([]string{subGroupId}, operationID)
	if err != nil {
		log.Error(operationID, "getGroupsInfoFromSvr failed ", err.Error())
		return
	}
	if len(groupInfos) == 0 {
		log.Error(operationID, "getGroupsInfoFromSvr failed empty")
		return
	}
	onServer := common.TransferToLocalGroupInfo(groupInfos)
	if err != nil {
		log.NewError(operationID, "GetJoinedGroupList failed ", err.Error())
		return
	}

	err = g.db.InsertGroup(onServer[0])
	if err != nil {
		log.NewError(operationID, "InsertGroup failed ", err.Error(), *onServer[0])
		return
	}
	isReadDiffusion := false
	callbackData := sdk.JoinedGroupAddedCallback(*onServer[0])
	if (*onServer[0]).GroupType == int32(constant.WorkingGroup) {
		isReadDiffusion = true
	}
	if g.listener != nil {
		g.listener.OnJoinedGroupAdded(utils.StructToJsonString(callbackData))
