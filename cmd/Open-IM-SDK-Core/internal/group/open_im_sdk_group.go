...
func (g *Group) SubscribeGroupChannel(callback open_im_sdk_callback.Base, groupID, operationID string) {
	if callback == nil {
		return
	}
	fName := utils.GetSelfFuncName()
	go func() {
		if groupID == "" {
			log.NewError(operationID, fName, "groupID is empty")
			callback.OnError(constant.ErrEmptyGroupID.ErrCode, constant.ErrEmptyGroupID.ErrMsg)
			return
		}
		log.NewInfo(operationID, fName, "args: ", groupID)
		g.subscribeGroupChannel(groupID, callback, operationID)
		callback.OnSuccess(utils.StructToJsonString(sdk_params_callback.SubGroupCallback))
		log.NewInfo(operationID, fName, "callback: ", utils.StructToJsonString(sdk_params_callback.SubGroupCallback))
	}()
}

func (g *Group) UnSubscribeGroupChannel(callback open_im_sdk_callback.Base, groupID, operationID string) {
	if callback == nil {
		return
	}
	fName := utils.GetSelfFuncName()
	go func() {
		log.NewInfo(operationID, fName, "args: ", groupID)
		g.unSubscribeGroupChannel(groupID, callback, operationID)
		callback.OnSuccess(utils.StructToJsonString(sdk_params_callback.UnSubGroupCallback))
		log.NewInfo(operationID, fName, "callback: ", utils.StructToJsonString(sdk_params_callback.UnSubGroupCallback))
	}()
}
...
func (g *Group) GetJoinedGroupList(callback open_im_sdk_callback.Base, operationID string) {
	...
	go func() {
		...
		groupList := g.getJoinedGroupListEx(callback, operationID)
...
func (g *Group) GetGroupChannelList(callback open_im_sdk_callback.Base, groupID, operationID string) {
	if callback == nil {
		return
	}
	fName := utils.GetSelfFuncName()
	go func() {
		log.NewInfo(operationID, fName, "args: ")
		groupList := g.getGroupChannelList(callback, groupID, operationID)
		log.Debug(operationID, "this is a dbd test", groupList)
		callback.OnSuccess(utils.StructToJsonStringDefault(groupList))
		log.NewInfo(operationID, fName, " callback: ", utils.StructToJsonStringDefault(groupList))
	}()
...
// GetJoinedGroupListFromApi 从api获取用户群列表
func (g *Group) GetJoinedGroupListFromApi(callback open_im_sdk_callback.Base, operationID string) {
	if callback == nil {
		return
	}
	fName := utils.GetSelfFuncName()
	go func() {
		...
		groupList := sdk.GetJoinedGroupListCallback{}
		...
		g.p.PostReturn(constant.GetJoinedGroupListRouter, apiReq, &result)
		for _, v := range result {
			dst := &model_struct.LocalGroup{}
			copier.Copy(dst, &v)
			groupList = append(groupList, dst)
		}

		log.Debug(operationID, "this is a api result ", result)
		log.Debug(operationID, "this is a copy list", groupList)
		callback.OnSuccess(utils.StructToJsonStringDefault(groupList))
		log.NewInfo(operationID, fName, " callback: ", utils.StructToJsonStringDefault(groupList))
	}()
}
...
func (g *Group) GetGroupMemberListFromServer(callback open_im_sdk_callback.Base, groupID string, filter, offset, count int32, operationID string) {
	fName := utils.GetSelfFuncName()
	go func() {
		log.NewInfo(operationID, fName, "args: ", groupID, offset, count)
		...
		apiReq.OperationID = operationID
		apiReq.GroupID = groupID
		apiReq.Offset = offset
		apiReq.Count = count
		var realData []*api.GroupMemberFullInfo
		err := g.p.PostReturn(constant.GetGroupAllMemberListRouter, apiReq, &realData)
		if err != nil {
			log.Error(operationID, "GetGroupMemberListFromServer failed ", constant.GetGroupAllMemberListRouter, apiReq)
		}
		log.Info(operationID, "GetGroupMemberListFromServer result len: ", len(realData), groupID)
		callback.OnSuccess(utils.StructToJsonStringDefault(common.TransferToLocalGroupMember(realData)))
		log.NewInfo(operationID, fName, " callback: ", utils.StructToJsonStringDefault(realData))
	}()
}
