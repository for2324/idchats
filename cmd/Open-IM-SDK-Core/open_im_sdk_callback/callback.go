...
type OnGroupListener interface {
	...
	OnGroupChannelInfoAdded(groupInfo string)
	OnGroupChannelInfoDelete(groupInfo string)
	OnGroupChannelInfoUpdate(groupInfo string)
}
type OnFriendshipListener interface {
	OnFollowFriendApplicationAdded(friendApplication string)
	OnFollowFriendApplicationDeleted(friendApplication string)
...
type OnConversationListener interface {
	...
	OnTotalGroupUnreadMessageCountChanged(totalUnreadCount int32)
}
type OnAdvancedMsgListener interface {
	...
	OnRecvMessageRevoked(msgID string)             //deprecated
	OnNewRecvMessageRevoked(messageRevoked string) //deprecated
}
...
type OnUserListener interface {
	...
	OnSelfScoreInfoUpdated(userScore string)
	OnUserOrderPay(payInfo string)
	OnUserGlobalAnnouncement(announce string)
}
